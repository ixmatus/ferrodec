...
//!
//! The extended-precision `*_ext()` accessors (consumed by the
//! transcendental kernels) moved to `ferrodec_transcend::consts` in
//! P0a.2 c4 and are re-exported here so the still-in-core kernels
//! keep their `crate::math::consts::*_ext` import unchanged until
//! they migrate to the shared crate.

use crate::decimal::Decimal128;
use crate::status::RoundingMode;

// The extended-precision constants now live in the shared
// `ferrodec-transcend` crate. Re-exported under the original paths so
// the kernels (still in core until c5..c11) compile unchanged. Which
// of these is "used" depends on the enabled transcendental
// sub-feature(s), so the re-export tolerates the per-feature unused
// analysis at the group level rather than gating each name.
#[allow(unused_imports)]
pub(crate) use ferrodec_transcend::consts::{
    e_ext, inv_ln10_ext, inv_ln2_ext, ln10_ext, ln2_ext, pi_ext, pi_over_four_ext, pi_over_two_ext,
    tan_pi_over_eight_ext,
};
