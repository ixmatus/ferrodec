...
/// [`Decimal128`], as returned by [`Decimal128::decode`].
///
...
/// distinct `Decimal128Parts`. Use [`Decimal128::canonicalize`] first if a
/// normalized cohort is wanted.
#[derive(Clone, Copy, Debug, PartialEq, Eq, Hash)]
pub struct Decimal128Parts {
    /// `true` when the value carries a negative sign, including `−0`.
    pub negative: bool,
    /// The integer coefficient (significand), in `[0, 10^34)`.
    pub coefficient: u128,
    /// The unbiased quantum exponent, in `[−6176, 6111]`.
    pub exponent: i16,
}
