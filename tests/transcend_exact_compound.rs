...
//! anchor-band gate for `Decimal128`'s `compound` (IEEE 754-2019 §9.2;
//! ADR-0059 Track D group D3, fd-4zo.25).
//!
...
/// `10^6144` is the last power of ten inside `Decimal128`.
const EMAX: i32 = 6144;
/// `10^-6176` is the smallest positive subnormal.
const ETINY: i32 = -6176;
/// The integer the shared `exp` overflow gate does not catch:
/// `6145 · ln 10 ≈ 14149.4` stays inside the format's 14150 limit while
/// `10^6145` is past `MAX`. Reached here as `compound(9, 6145)`, the
...
fn n_zero_is_one_for_the_whole_domain_and_quiet_nan() {
    for rm in ALL {
        for literal in [
            ...
            assert!(
                equal(r, Decimal128::ONE),
                "compound({literal}, 0) at {rm:?}: got {r}, want 1"
            );
            assert_eq!(st, Status::OK, "compound({literal}, 0) at {rm:?}: flags");
        }
        // +∞ ≥ −1, so it takes the 1 row too.
        let (r, st) = Decimal128::INFINITY.compound(0, rm);
        assert!(equal(r, Decimal128::ONE), "compound(+∞, 0) at {rm:?}: {r}");
        assert_eq!(st, Status::OK);
        // The quiet-NaN carve-out.
        let (r, st) = Decimal128::NAN.compound(0, rm);
        assert!(
            equal(r, Decimal128::ONE),
            "compound(qNaN, 0) at {rm:?}: {r}"
        );
...
fn below_negative_one_is_invalid_at_every_n() {
    for rm in ALL {
        for n in [i32::MIN, -7, -1, 0, 1, 7, i32::MAX] {
            ...
            // `Decimal128` gets to the domain edge from below.
            for literal in ["-1.000000000000000000000000000000001", "-2", "-1E+6144"] {
            ...
            let (r, st) = Decimal128::NEG_INFINITY.compound(n, rm);
...
fn positive_infinity_follows_the_sign_of_n() {
    for rm in ALL {
        for n in [1, 2, i32::MAX] {
            let (r, st) = Decimal128::INFINITY.compound(n, rm);
            assert!(
                r.is_infinite() && !r.is_sign_negative(),
...
/// The independent witness for the exact family: raising the delivered
/// value back by `pow` reproduces `1 + x`. A different kernel and a
/// different classifier (`exact::pow_exact_input`, on the
/// Lauter–Lefèvre criterion) reaching the same rational, so agreement is
/// evidence rather than tautology.
#[cfg(feature = "pow")]
#[test]
fn pow_takes_the_exact_family_back() {
    for (x, n) in [("0.05", 3), ("9", 2), ("0.25", -1), ("5", 2), ("-0.5", 3)] {
        let (v, _) = parse(x).compound(n, NE);
        let inv = Decimal128::ONE.div(Decimal128::from_i32(n), NE).0;
        let (back, _) = v.pow(inv, NE);
        let (base, _) = Decimal128::ONE.add(parse(x), NE);
        assert!(
            equal(back, base),
            "pow(compound({x}, {n}), 1/{n}) = {back}, want 1 + {x} = {base}"
        );
    }
}
...
fn nines_family_across_k_and_both_signs_of_n() {
    for (x, k) in [
        ...
        ("-0.99", -2),
        ("-0.999", -3),
        ("-0.9999999999999999999999999999999999", -34),
    ] {
...
fn above_range_powers_of_ten_overflow_per_mode() {
    let max = parse("9.999999999999999999999999999999999E+6144");
...
fn below_range_powers_of_ten_underflow_per_mode() {
    ...
    for n in -ETINY + 1..=-ETINY + 40 {
        ...
        assert!(
            equal(r, tiny),
            "compound(−0.9, {n}) at TowardPositive: got {r}, want 1E−6176"
        );
...
/// gives `compound(4, n) = 5^n`, and `5^49` / `5^50` are the two powers
/// of five with 35 digits. `compound(1, −49) = 2^−49 = 5^49 · 10^−49`
/// mirrors the first from the negative side.
///
...
/// `…312`, `NearestAway` and `TowardPositive` up to `…313`, `TowardZero`
/// and `TowardNegative` truncating to `…312`.
#[test]
fn nearest_mode_ties_resolve_by_the_rounders_own_rule() {
    let down = "1.776356839400250464677810668945312E+34";
    let up = "1.776356839400250464677810668945313E+34";
    for (rm, want) in [(NE, down), (NA, up), (TZ, down), (TP, up), (TN, down)] {
        let (r, st) = parse("4").compound(49, rm);
        assert_eq!(text(r), want, "compound(4, 49) at {rm:?}");
        assert_eq!(st, Status::INEXACT, "a tie is inexact in every direction");
    }
    // 5^50 = 8.8817841970012523233890533447265625E+34: last kept digit
    // is 2 again, so NearestEven again keeps.
    let down50 = "8.881784197001252323389053344726562E+34";
    let up50 = "8.881784197001252323389053344726563E+34";
    for (rm, want) in [
        (NE, down50),
        (NA, up50),
        (TZ, down50),
        ...
        let (r, st) = parse("4").compound(50, rm);
        assert_eq!(text(r), want, "compound(4, 50) at {rm:?}");
        assert_eq!(st, Status::INEXACT);
    }
    // The negative-`n` mirror: 2^-49 carries the same 35-digit
    // coefficient at exponent −49.
    let down_neg = "1.776356839400250464677810668945312E-15";
    let up_neg = "1.776356839400250464677810668945313E-15";
    for (rm, want) in [
        (NE, down_neg),
        (NA, up_neg),
        (TZ, down_neg),
        ...
        let (r, st) = parse("1").compound(-49, rm);
        assert_eq!(text(r), want, "compound(1, −49) at {rm:?}");
        assert_eq!(st, Status::INEXACT);
    }
    // One past the tie window the coefficient needs 36 digits, so the
    // classifier declines and the ladder rounds it: still inexact, and
    // still correct, but by a different mechanism.
    let (_, st) = parse("4").compound(51, NE);
    assert_eq!(st, Status::INEXACT);
}
...
/// moves to `next_up(1) = 1 + 10^−33`; below 1, `TowardZero` and
/// `TowardNegative` move to `1 − 10^−34`.
#[test]
fn anchor_band_delivers_by_the_side_theorem() {
    let one = "1.000000000000000000000000000000000";
    let next_up = "1.000000000000000000000000000000001";
    let next_down = "0.9999999999999999999999999999999999";
    // sign(n) = sign(x): the value is strictly above 1.
    for (x, n) in [("1E-40", 1), ("-1E-40", -1), ("1E-45", 7), ("-1E-50", -3)] {
...
/// multiplier cannot smuggle a resolvable value into it. The value
/// `1 + 2.147…e−31` is many boundaries away from 1 and the ladder
...
fn a_huge_n_leaves_the_anchor_band() {
    let (r, st) = parse("1E-40").compound(i32::MAX, NE);
    assert_eq!(text(r), "1.000000000000000000000000000000215");
    assert_eq!(st, Status::INEXACT);
    let (r, _) = parse("-1E-40").compound(i32::MAX, NE);
    assert_eq!(text(r), "0.9999999999999999999999999999997853");
}
...
/// existed, `compound(1E+200, 1)` at `TowardPositive` delivered
/// `10^200` where the true value `10^200 + 1` is strictly above it and
/// `next_up(10^200)` is owed; `compound(1E+2000, 1)`,
/// `compound(1E+200, 2)` and `compound(1E+2000, 3)` were the same
/// shape. `compound(3E+200, 1)` happened to come out right, which is
...
/// The threshold-straddling pair. At `Decimal128` with a one-digit `n`
...
fn the_gate_threshold_does_not_move_the_answer() {
    for n in [1i32, -1] {
        for rm in ALL {
            let (inside, st_in) = parse("1E-39").compound(n, rm);
            let (outside, st_out) = parse("1E-38").compound(n, rm);
            assert_eq!(
                text(inside),
                text(outside),
                "compound(1E−39, {n}) and compound(1E−38, {n}) at {rm:?} \
...
fn n_one_is_the_sum() {
    for literal in ["0.05", "1", "9", "-0.5", "1E-30", "-1E-30", "123.456"] {
        let x = parse(literal);
        let (want, want_st) = Decimal128::ONE.add(x, NE);
