...
//! `Decimal128::sin_pi` (IEEE 754-2019 §9.2 `sinPi`; ADR-0061 Track D
//! group D4).
//!
...
//! is the claim's witness at 34 digits.
//!
...
//! `1/2`, so the stored quantum forces the offset to `10^-34` or more
//! and the hug bottoms out around `4.9·10^-68`, decades outside every
...
/// `sin(54°) = cos(36°) = φ/2`, the golden ratio halved, at 34 digits.
const SIN_54: &str = "0.8090169943749474241022934171828191";
/// `sin(18°) = (√5 − 1)/4` at 34 digits.
const SIN_18: &str = "0.3090169943749474241022934171828191";
/// `sin(45°) = √2/2` at 34 digits (the same constant
/// `ferrodec_transcend::rsqrt`'s tests carry to 130 places).
const SIN_45: &str = "0.7071067811865475244008443621048490";
...
fn specials_per_section_9_2_1() {
    for rm in ALL {
        // sinPi(±0) is ±0.
        assert_zero(
            Decimal128::ZERO.sin_pi(rm),
            false,
            &format!("sinPi(+0) [{rm:?}]"),
        );
        assert_zero(
            Decimal128::NEG_ZERO.sin_pi(rm),
            true,
            &format!("sinPi(-0) [{rm:?}]"),
        );
        ...
        let (r, st) = Decimal128::NAN.sin_pi(rm);
        assert!(r.is_nan() && st.is_ok(), "sinPi(NaN) [{rm:?}] = {r} {st:?}");
        let (r, st) = Decimal128::SIGNALING_NAN.sin_pi(rm);
        assert!(
            r.is_nan() && st.invalid(),
            "sinPi(sNaN) [{rm:?}] = {r} {st:?}"
        );
...
fn exact_at_the_half_integers() {
    ...
    for (label, want_pos) in [
        ...
        ("2.50", &one),
        ("2.500000000000000000000000000000000", &one),
        ("0.50", &one),
        ("250E-2", &one),
        // The widest half integer this format can spell.
        ("999999999999999999999999999999999.5", &neg_one),
    ] {
        for rm in ALL {
            assert_exact(
                parse(label).sin_pi(rm),
...
fn small_arguments_track_the_pi_slope() {
    ...
    for label in [
        "1E-20", "1E-40", "1E-100", "1E-3000", "1E-6100", "0.000001", "1E-6176",
    ] {
        let x = parse(label);
        for rm in ALL {
            let (r, st) = x.sin_pi(rm);
...
/// `5·10^-35`, so nearest delivers exactly `1` once `δ < 3.2·10^-18`
...
/// `10^-34`, where the hug is `≈ 4.9·10^-68`: bounded well away from
/// zero, which is exactly why no anchor channel is needed.
#[test]
fn half_integer_hug_needs_no_anchor() {
    let one = parse("1");
    let just_below_one = parse("0.9999999999999999999999999999999999");
    // (operand, positive lobe, nearest rounds all the way to 1)
    let cases: [(&str, bool, bool); 6] = [
        ("0.5000000000000000000000000000000001", true, true),
        ("0.4999999999999999999999999999999999", true, true),
        ("0.500000000000000001", true, true),
        // 34 digits wide, so the offset survives the parse; a 35th
        // would round the operand back onto the exact half integer.
        ("1.500000000000000000000000000000001", false, true),
        // δ = 10^-17 puts the hug at 4.9·10^-34, past the boundary.
        ("0.50000000000000001", true, false),
        ("0.49999999999999999", true, false),
    ];
    for (label, positive_lobe, snaps_to_one) in cases {
        let x = parse(label);
        let sign = |v: Decimal128| if positive_lobe { v } else { v.neg() };
        for rm in [NE, NA] {
            let (r, st) = x.sin_pi(rm);
            assert!(st.inexact(), "sinPi({label}) [{rm:?}] must be INEXACT");
            if snaps_to_one {
