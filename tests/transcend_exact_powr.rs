...
fn the_ieee_9_2_2_preferred_exponent_deviation() {
    ...
    // `pow`'s own inconsistency across its fast-path boundary. Both
    // exponents want quantum floor(y × 2); pow answers them
    // differently, and powr matches pow exactly where pow uses the
    // shared classifier.
    ...
    assert_eq!(
        alloc_string(p257),
        "1.000000000000000000000000000000000E+514",
...
fn differential_against_pow_over_the_shared_domain() {
    ...
    assert_eq!(
        ...
        "the cohort-split bucket drifted from its ten pinned pairs"
    );
}
