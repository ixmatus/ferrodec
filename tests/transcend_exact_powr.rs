...
//! for `Decimal128`'s `powr` (IEEE 754-2019 §9.2; ADR-0059 Track D D3).
//!
...
fn finite_positive_base_zero_exponent_is_one() {
    for x in ["2", "0.5", "1E+300", "1E-300", "123.456", "9.5"] {
        for z in [Decimal128::ZERO, Decimal128::NEG_ZERO] {
            for rm in ALL {
                let (r, s) = parse(x).powr(z, rm);
                assert!(equal(r, Decimal128::ONE), "powr({x}, ±0) = 1, got {r:?}");
...
fn zero_base_negative_exponent_is_infinity() {
    for z in [Decimal128::ZERO, Decimal128::NEG_ZERO] {
        ...
        for rm in ALL {
            let (r, s) = z.powr(Decimal128::NEG_INFINITY, rm);
            assert!(
                r.is_infinite() && !r.is_sign_negative(),
                "powr(±0, −∞) = +∞"
            );
...
fn zero_base_positive_exponent_is_positive_zero() {
    for z in [Decimal128::ZERO, Decimal128::NEG_ZERO] {
        ...
        let (r, s) = z.powr(Decimal128::INFINITY, NE);
        assert!(r.is_zero() && !r.is_sign_negative(), "powr(±0, +∞) = +0");
        assert_eq!(s, Status::OK);
    }
    // The sign-rule contrast with pow, pinned.
    let (pr, _) = Decimal128::NEG_ZERO.pow(parse("3"), NE);
    assert!(
        pr.is_sign_negative(),
        "pow(−0, 3) = −0 by the odd-integer rule"
    );
    let (qr, _) = Decimal128::NEG_ZERO.powr(parse("3"), NE);
...
fn unit_base_finite_exponent_is_one() {
    for y in ["1", "-1", "0.5", "-3.14", "1E+300", "-1E-300"] {
        for rm in ALL {
            let (r, s) = Decimal128::ONE.powr(parse(y), rm);
            assert!(equal(r, Decimal128::ONE), "powr(1, {y}) = 1, got {r:?}");
...
fn negative_base_is_invalid_for_every_exponent() {
    let ys = ["3", "2", "-3", "0.5", "-0.5", "1", "0", "-0", "1E+300"];
    for x in ["-2", "-1", "-0.5", "-1E+300"] {
        for y in ys {
            for rm in ALL {
    ...
    let (r, s) = Decimal128::NEG_INFINITY.powr(parse("2"), NE);
    assert!(r.is_quiet_nan() && s.invalid(), "powr(−∞, 2) is invalid");
    let (r, s) = Decimal128::NEG_ZERO.powr(parse("2"), NE);
...
fn the_three_indeterminate_forms_contrast_with_pow() {
    let zeros = [Decimal128::ZERO, Decimal128::NEG_ZERO];
    let infs = [Decimal128::INFINITY, Decimal128::NEG_INFINITY];
    ...
    for x in zeros {
        for y in zeros {
            ...
            assert!(equal(pr, Decimal128::ONE), "pow(±0, ±0) = 1");
    ...
    for y in zeros {
        for rm in ALL {
            let (r, s) = Decimal128::INFINITY.powr(y, rm);
            assert!(r.is_quiet_nan(), "powr(+∞, ±0) = qNaN, got {r:?}");
            assert!(s.invalid(), "powr(+∞, ±0) signals INVALID");
        }
        let (pr, ps) = Decimal128::INFINITY.pow(y, NE);
        assert!(equal(pr, Decimal128::ONE), "pow(+∞, ±0) = 1");
    ...
    for y in infs {
        for rm in ALL {
            let (r, s) = Decimal128::ONE.powr(y, rm);
            assert!(r.is_quiet_nan(), "powr(+1, ±∞) = qNaN, got {r:?}");
            assert!(s.invalid(), "powr(+1, ±∞) signals INVALID");
        }
        let (pr, ps) = Decimal128::ONE.pow(y, NE);
        assert!(equal(pr, Decimal128::ONE), "pow(+1, ±∞) = 1");
        assert!(!ps.invalid(), "pow(+1, ±∞) raises nothing");
        // pow(−1, ±∞) is also 1; powr refuses it as a negative base.
        let (pr, ps) = Decimal128::NEG_ONE.pow(y, NE);
        assert!(equal(pr, Decimal128::ONE), "pow(−1, ±∞) = 1");
        assert!(!ps.invalid());
        let (qr, qs) = Decimal128::NEG_ONE.powr(y, NE);
...
fn infinite_operand_limits_follow_the_composition() {
    ...
    for y in ["1", "2", "0.5", "1E+300"] {
        let (r, s) = Decimal128::INFINITY.powr(parse(y), NE);
        assert!(
            r.is_infinite() && !r.is_sign_negative(),
            "powr(+∞, {y}) = +∞"
        );
        assert_eq!(s, Status::OK);
        let (r, s) = Decimal128::INFINITY.powr(parse(&format!("-{y}")), NE);
        assert!(r.is_zero() && !r.is_sign_negative(), "powr(+∞, -{y}) = +0");
        assert_eq!(s, Status::OK);
    }
    let (r, _) = Decimal128::INFINITY.powr(Decimal128::INFINITY, NE);
    assert!(r.is_infinite(), "powr(+∞, +∞) = +∞");
    let (r, _) = Decimal128::INFINITY.powr(Decimal128::NEG_INFINITY, NE);
    assert!(r.is_zero(), "powr(+∞, −∞) = +0");

    // powr(x, ±∞) for finite x > 0, x ≠ 1: ln x sets the sign.
    for x in ["2", "10", "1.5", "1E+300"] {
        let (r, s) = parse(x).powr(Decimal128::INFINITY, NE);
        assert!(
            r.is_infinite() && !r.is_sign_negative(),
            "powr({x}>1, +∞) = +∞"
        );
        assert_eq!(s, Status::OK);
        let (r, s) = parse(x).powr(Decimal128::NEG_INFINITY, NE);
        assert!(r.is_zero() && !r.is_sign_negative(), "powr({x}>1, −∞) = +0");
        assert_eq!(s, Status::OK);
    }
    for x in ["0.5", "0.1", "0.99999", "1E-300"] {
        let (r, s) = parse(x).powr(Decimal128::INFINITY, NE);
        assert!(r.is_zero() && !r.is_sign_negative(), "powr({x}<1, +∞) = +0");
        assert_eq!(s, Status::OK);
        let (r, s) = parse(x).powr(Decimal128::NEG_INFINITY, NE);
...
fn nan_rows_have_no_preempting_row() {
    for x in ["0", "-0", "1", "2", "1E+300"] {
        let (r, s) = parse(x).powr(Decimal128::NAN, NE);
        assert!(r.is_quiet_nan(), "powr({x}, qNaN) = qNaN");
        assert_eq!(s, Status::OK, "powr({x}, qNaN) raises nothing");
        let (r, s) = parse(x).powr(Decimal128::SIGNALING_NAN, NE);
        assert!(r.is_quiet_nan(), "powr({x}, sNaN) = qNaN");
        assert!(s.invalid(), "powr({x}, sNaN) signals INVALID");
    }
    for y in ["0", "-0", "1", "2", "-2"] {
        let (r, s) = Decimal128::NAN.powr(parse(y), NE);
        assert!(r.is_quiet_nan(), "powr(qNaN, {y}) = qNaN");
        assert_eq!(s, Status::OK, "powr(qNaN, {y}) raises nothing");
        let (r, s) = Decimal128::SIGNALING_NAN.powr(parse(y), NE);
        assert!(r.is_quiet_nan(), "powr(sNaN, {y}) = qNaN");
        assert!(s.invalid(), "powr(sNaN, {y}) signals INVALID");
    }
    // NaN in both operands, and the ±∞ pairing.
    let (r, s) = Decimal128::NAN.powr(Decimal128::NAN, NE);
    assert!(r.is_quiet_nan() && s == Status::OK);
    let (r, s) = Decimal128::NAN.powr(Decimal128::SIGNALING_NAN, NE);
    assert!(r.is_quiet_nan() && s.invalid());

    // The contrast: pow's rules 1 and 2 fire through NaN, powr's do not.
    let (pr, _) = Decimal128::NAN.pow(Decimal128::ZERO, NE);
    assert!(equal(pr, Decimal128::ONE), "pow(NaN, 0) = 1");
    let (qr, _) = Decimal128::NAN.powr(Decimal128::ZERO, NE);
    assert!(qr.is_quiet_nan(), "powr(NaN, 0) = qNaN");
    let (pr, _) = Decimal128::ONE.pow(Decimal128::NAN, NE);
    assert!(equal(pr, Decimal128::ONE), "pow(1, NaN) = 1");
    let (qr, _) = Decimal128::ONE.powr(Decimal128::NAN, NE);
...
/// The real `PRECISION + 1` tie: `5^49` is 35 digits ending in 5, one
/// digit past `Decimal128`'s 34, so its true value sits exactly on a
...
///
/// `5^49 = 17763568394002504646778106689453125`; the two neighbours are
/// `...45312E+1` (even) and `...45313E+1`.
#[test]
fn the_five_49_tie_resolves_per_mode() {
    let x = parse("5");
    let y = parse("49");
    let down = parse("1776356839400250464677810668945312E+1");
    let up = parse("1776356839400250464677810668945313E+1");
    let expect = [
    ...
    for (rm, want, why) in expect {
        let (r, s) = x.powr(y, rm);
        assert!(equal(r, want), "powr(5, 49) under {rm:?}: {why}; got {r:?}");
        assert!(s.inexact(), "powr(5, 49) is a tie: INEXACT in every mode");
        // The tie is the classifier's, so powr and pow must agree on it.
        let (pr, ps) = x.pow(y, rm);
        assert!(
            equal(r, pr),
            "powr and pow agree on the 5^49 tie under {rm:?}"
        );
...
/// rounder moves the cohort toward it as far as the 34-digit
/// coefficient allows (§6.3). Every case below is therefore either the
/// §9.2.2 quantum verbatim or its §6.3 clamp; see
...
fn exact_deliveries_pin_their_quantum() {
    ...
    let cases = [
        ...
        ("1.5", "3", "3.375"),
        // Q(10) = 0, y = 300 → floor = 0; 10^300 needs a 301-digit
        // coefficient to sit at quantum 0, so the clamp lands on the
        // widest representable cohort, quantum 267.
        ("10", "300", "1.000000000000000000000000000000000E+300"),
...
/// same one, so `y = 256` and `y = 257` land in one cohort family.
#[test]
fn the_ieee_9_2_2_preferred_exponent_conformance() {
    ...
    // The fast-path boundary witness, both sides now one family.
    let (p256, _) = parse("1E+2").pow(parse("256"), NE);
    let (p257, _) = parse("1E+2").pow(parse("257"), NE);
    assert_eq!(
        alloc_string(p256),
        "1E+512",
        "pow's fast path keeps the 1 cohort"
    );
    assert_eq!(
        alloc_string(p257),
        "1E+514",
    ...
    let (q257, _) = parse("1E+2").powr(parse("257"), NE);
    assert_eq!(
        alloc_string(q257),
        alloc_string(p257),
        "off the fast path, powr and pow deliver the identical cohort"
    );
}

/// `Display` render, the cohort-sensitive view the quantum pins need.
fn alloc_string(d: Decimal128) -> std::string::String {
    std::format!("{d}")
}
...
fn differential_against_pow_over_the_shared_domain() {
    ...
    // fd-5g6 emptied the cohort-split bucket: the classifier now
    // delivers the same §9.2.2 preferred quantum as pow's fast path,
    // so the ten power-of-ten pairs that used to differ in cohort
    // alone (`("0.1", "-1")`, `("1E+50", "2")`, …) are bit-identical.
