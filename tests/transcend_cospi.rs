...
//! `Decimal128::cos_pi` (IEEE 754-2019 §9.2 `cosPi`; ADR-0061 Track D
//! group D4).
//!
...
//! operand itself and reaches `10^-6176`. The kernel gates the channel
//! at `adj(δ) ≤ −⌈(P + 4)/2⌉`, which is `−19` at this format, and
...
//! the last nearest-mode boundary below 1 (`5·10^-35`) once
//! `|δ| < 3.2·10^-18`, so nearest delivers exactly `±1` from there
...
/// `cos(36°) = φ/2` at 34 digits.
const COS_36: &str = "0.8090169943749474241022934171828191";
/// `cos(54°) = sin(36°)` at 34 digits.
const COS_54: &str = "0.5877852522924731291687059546390728";
/// `cos(45°) = √2/2` at 34 digits.
const COS_45: &str = "0.7071067811865475244008443621048490";
/// The largest `Decimal128` strictly below 1.
const JUST_BELOW_ONE: &str = "0.9999999999999999999999999999999999";
...
fn specials_per_section_9_2_1() {
    ...
    for rm in ALL {
        // cosPi(±0) is 1.
        assert_exact(
            Decimal128::ZERO.cos_pi(rm),
            one,
            &format!("cosPi(+0) [{rm:?}]"),
        );
        assert_exact(
            Decimal128::NEG_ZERO.cos_pi(rm),
            one,
            &format!("cosPi(-0) [{rm:?}]"),
        );
        ...
        let (r, st) = Decimal128::NAN.cos_pi(rm);
        assert!(r.is_nan() && st.is_ok(), "cosPi(NaN) [{rm:?}] = {r} {st:?}");
        let (r, st) = Decimal128::SIGNALING_NAN.cos_pi(rm);
        assert!(
            r.is_nan() && st.invalid(),
            "cosPi(sNaN) [{rm:?}] = {r} {st:?}"
        );
...
fn exact_at_the_integers() {
    ...
    for (label, even) in [
        ...
        ("1E+40", true),
        ("1E+6100", true),
        ("9.999999999999999999999999999999999E+6144", true),
        // A full-width odd integer.
        ("9999999999999999999999999999999999", false),
        // Cohorts of an even integer.
        ("2.0", true),
        ("2.000000000000000000000000000000000", true),
        ("200E-2", true),
    ] {
...
fn exact_at_the_half_integers_is_always_positive_zero() {
    for label in [
        ...
        // Cohort variants.
        "2.50",
        "2.500000000000000000000000000000000",
        "0.50",
        "250E-2",
...
fn known_values_against_closed_forms() {
    for (label, want) in [
        ...
        ("0.8", "-0.8090169943749474241022934171828191"),
        ("0.7", "-0.5877852522924731291687059546390728"),
        ("0.75", "-0.7071067811865475244008443621048490"),
    ] {
...
///
/// The kernel's channel opens at `adj(δ) ≤ −19`. Nothing observable may
/// change there, so this walks `k` from well above the gate (`10^-18`,
/// ladder territory) down through it (`10^-19` and below) to the
...
fn anchor_band_across_the_gate() {
    ...
    // 10^-18 is one decade ABOVE the gate and still inside the last
    // boundary below 1; 10^-19 is the first gated decade.
    for k in [
        18i32, 19, 20, 21, 25, 34, 40, 100, 1000, 3000, 6100, 6144, 6176,
    ] {
...
fn anchor_band_beside_the_integers() {
    ...
    let cases: [(&str, bool); 4] = [
        ...
        ("2.000000000000000000000000000000001", true),
        ("1.999999999999999999999999999999999", true),
    ];
    for (label, positive) in cases {
        let x = parse(label);
        let sign = |v: Decimal128| if positive { v } else { v.neg() };
        for rm in [NE, NA] {
            let (r, st) = x.cos_pi(rm);
            assert!(
                eq(r, sign(one)),
...
/// `3.2·10^-18` regime boundary and proves the band test above is not
/// vacuously asserting 1 everywhere.
#[test]
fn wide_offsets_do_not_reach_the_anchor() {
    let one = parse("1");
    for label in ["1E-17", "1E-10", "0.000001", "0.0001"] {
        let x = parse(label);
        for rm in ALL {
            let (r, st) = x.cos_pi(rm);
            assert!(st.inexact(), "cosPi({label}) [{rm:?}] must be INEXACT");
    ...
    // 10^-4 is the tightest nearest-mode call in the family at the
    // narrow formats; here it is comfortably resolved.
    let (r, _) = parse("0.0001").cos_pi(NE);
    assert!(eq(r, parse("0.9999999506519784004244182122416003")));
}
