//! Integer-argument, special-value, and flag gate for `Decimal128`'s
//! `exp10_m1` (IEEE 754-2019 §9.2 `exp10m1`; ADR-0059 Track D).
//!
...
//! overflow gate and the true overflow boundary (`n = 6145` here)
...
/// `Decimal128` significand width: the exact family's `|n|` ceiling.
const PRECISION: u32 = 34;
/// `Decimal128`'s largest exponent: `10^E_MAX` is representable,
/// `10^(E_MAX + 1)` is not. `E_MAX + 1` is also the gap integer
/// between the kernel's overflow gate (which fires above
/// `x ≈ 6145.19`) and the true overflow boundary.
...
fn positive_neighbours(n: i32) -> (Decimal128, Decimal128) {
...
/// answers.
fn negative_neighbours() -> (Decimal128, Decimal128) {
    let away = parse("-1");
...
fn coarse_integer_rows_every_mode() {
    for n in [
        ...
        -200,
        -1000,
        -6176, // etiny scale: the result still sits just above −1
        -99_999,
        -100_000,
    ] {
        check_integer(n);
...
/// is the 34 nines value at that decade, written out here in full.
#[test]
fn rung2_misround_family_witness_at_200() {
    let x = parse("200");
    let want_toward = parse("9.999999999999999999999999999999999e199");
...
fn subnormal_results_raise_underflow() {
    let (r, st) = parse("1e-6170").exp10_m1(NE);
    assert!(r.is_subnormal(), "exp10_m1(1e-6170) is subnormal, got {r}");
    assert!(
        st.underflow() && st.inexact(),
        "exp10_m1(1e-6170): want UNDERFLOW | INEXACT"
    );
...
fn inexact_flag_is_honest_in_every_mode() {
    ...
    for literal in [
        "2", "2.000", "9", "34", "3.4e1", "-2", "-2.000", "-34", "-3.4e1",
    ] {
        let x = parse(literal);
        for rm in ALL {
            let (_, st) = x.exp10_m1(rm);
...
fn special_values_every_mode() {
    for rm in ALL {
        let (r, st) = Decimal128::ZERO.exp10_m1(rm);
        assert!(
        ...
        let (r, st) = Decimal128::NEG_ZERO.exp10_m1(rm);
        assert!(
            r.is_zero() && r.is_sign_negative(),
            "exp10_m1(-0) at {rm:?}"
        );
        ...
        let (r, st) = Decimal128::NEG_INFINITY.exp10_m1(rm);
        assert!(
        ...
        let (r, st) = Decimal128::INFINITY.exp10_m1(rm);
        assert!(
            r.is_infinite() && !r.is_sign_negative(),
            "exp10_m1(+inf) at {rm:?}"
        );
