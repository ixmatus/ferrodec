...
11. **S2 deep-margin campaign corpus** (fd-4zo.19, `tests/vectors/transcend/campaign/`). The hardest certified survivors of a 2.7-billion-sample local sweep over the priority set (sin, cos, tan, exp, ln, log10, sinh, cosh, pow at Decimal128; exp, sin, cos at Decimal64): at most 50 inputs per function and format ranked by exact boundary distance, every output re-derived through the Arb proof tier, replayed bit-exact with exact per-bucket pins. The campaign's mirror-divergence channel additionally certified all 198,534 cases where rung 2 corrected rung 1's rounding, each one confirmed correct against Arb: the escalation ladder observed doing its job at scale. `MANIFEST.json` records the campaign parameters (id, depth, threshold, seeds, git SHA) for citability.
