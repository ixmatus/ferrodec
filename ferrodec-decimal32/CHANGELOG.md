...
- A `telemetry` feature forwarding `ferrodec-transcend/telemetry`
  (ADR-0059 S3), and with it the structural zero pin: at P = 7
  the escalation ladder's rung-1 threshold sits below what the
  coefficient lattice can express, so no `Decimal32` input can
...
- `Decimal32::sin_pi`, `cos_pi`, `tan_pi`, `asin_pi`, `acos_pi`,
...
- The correctly rounded claim for `Decimal32::powi` (`−6 ≤ n ≤ 6`),
...
- `Decimal32::powi`, `powr`, `rootn`, `compound`, `rsqrt`, and
...
- `Decimal32::ln_1p`, `log2_1p`, and `log10_1p` (IEEE 754-2019 §9.2
  ...
  `exp-log`. Exact families (`log2_1p` at `2^k − 1` up to `k = 23`
  and the matching negative fractions; `log10_1p` at the nines
  patterns up to 7 digits) deliver with no `INEXACT` in every mode.
