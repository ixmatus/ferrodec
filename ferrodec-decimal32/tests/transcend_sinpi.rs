...
//! `Decimal32::sin_pi` (IEEE 754-2019 §9.2 `sinPi`; ADR-0061 Track D
//! group D4).
//!
...
//! is the claim's witness at 7 digits.
//!
...
//! `1/2`, so the stored quantum forces the offset to `10^-7` or more
//! and the hug bottoms out around `4.9·10^-14`, decades outside every
...
/// `sin(54°) = cos(36°) = φ/2`, the golden ratio halved, at 7 digits.
const SIN_54: &str = "0.8090170";
/// `sin(18°) = (√5 − 1)/4` at 7 digits.
const SIN_18: &str = "0.3090170";
/// `sin(45°) = √2/2` at 7 digits (the same constant
/// `ferrodec_transcend::rsqrt`'s tests carry to 130 places).
const SIN_45: &str = "0.7071068";
...
fn specials_per_section_9_2_1() {
    for rm in ALL {
        // sinPi(±0) is ±0.
        assert_zero(
            Decimal32::ZERO.sin_pi(rm),
            false,
            &format!("sinPi(+0) [{rm:?}]"),
        );
        assert_zero(
            Decimal32::NEG_ZERO.sin_pi(rm),
            true,
            &format!("sinPi(-0) [{rm:?}]"),
        );
        ...
        let (r, st) = Decimal32::NAN.sin_pi(rm);
        assert!(r.is_nan() && st.is_ok(), "sinPi(NaN) [{rm:?}] = {r} {st:?}");
        let (r, st) = Decimal32::SIGNALING_NAN.sin_pi(rm);
        assert!(
            r.is_nan() && st.invalid(),
            "sinPi(sNaN) [{rm:?}] = {r} {st:?}"
        );
...
fn small_arguments_track_the_pi_slope() {
    ...
    for label in [
        "1E-20", "1E-40", "1E-50", "1E-50", "1E-95", "0.000001", "1E-101",
    ] {
        let x = parse(label);
        for rm in ALL {
            let (r, st) = x.sin_pi(rm);
...
/// `5·10^-8`, so nearest delivers exactly `1` once `δ < 1.0·10^-4`
