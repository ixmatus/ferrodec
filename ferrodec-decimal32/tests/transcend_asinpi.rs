...
//! catches one. At `P = 7` the correctly rounded `1/6` is
//! `0.1666667` (the tail past seven digits is `0.666…` of
...
const P: usize = 7;
const SIXTH_UP: &str = "0.1666667";
const SIXTH_DOWN: &str = "0.1666666";
...
fn specials_per_section_9_2_1() {
    for rm in ALL {
        let (r, st) = Decimal32::ZERO.asin_pi(rm);
        assert!(r.is_zero() && !r.is_sign_negative(), "asinPi(+0) [{rm:?}]");
        assert_eq!(st, Status::OK, "asinPi(+0) [{rm:?}] flags");
        let (r, st) = Decimal32::NEG_ZERO.asin_pi(rm);
        assert!(r.is_zero() && r.is_sign_negative(), "asinPi(-0) [{rm:?}]");
        assert_eq!(st, Status::OK, "asinPi(-0) [{rm:?}] flags");

        for label in ["1.000001", "-1.5", "2", "-1e10"] {
        ...
        let (r, st) = Decimal32::NAN.asin_pi(rm);
        assert!(r.is_nan() && st.is_ok(), "asinPi(NaN) [{rm:?}] {r} {st:?}");
        let (r, st) = Decimal32::SIGNALING_NAN.asin_pi(rm);
...
/// The non terminating rational row at `P = 7`, pinned per
...
fn one_sixth_rows() {
    ...
    let sixth_long = format!("0.1{}", "6".repeat(30));
    for rm in ALL {
...
fn tiny_arguments_are_generic() {
    ...
    for label in ["1e-20", "1e-50", "3e-95"] {
        let x = parse(label);
        for rm in ALL {
            let (r, st) = x.asin_pi(rm);
...
/// No anchor at `±1` either: at `P = 7` the closest representable
/// input below 1 puts the value `sqrt(2·10^-7)/π ≈ 1.4·10^-4` below
/// `1/2`, three decades above the quantum `10^-7`. The square root
/// scale keeps halving the format's decades, which is why the claim
/// holds at every precision and not just the widest, and why the
/// separation is narrowest here.
#[test]
fn inputs_next_to_one_stay_decades_from_the_half_turn() {
    ...
    assert!(
        gap.partial_cmp(parse("1e-5")).0 == Some(Ordering::Greater)
            && gap.partial_cmp(parse("1e-3")).0 == Some(Ordering::Less),
...
fn asin_pi_plus_acos_pi_is_a_half_turn() {
    for label in ["0", "0.5", "-0.5", "1", "-1", "0.25", "-0.9", "1e-20"] {
        ...
        assert!(
            diff.abs().partial_cmp(parse("1e-5")).0 == Some(Ordering::Less),
