...
//! anchor-band gate for `Decimal32`'s `compound` (IEEE 754-2019 §9.2;
...
/// `10^96` is the last power of ten inside `Decimal32`.
const EMAX: i32 = 96;
/// `10^-101` is the smallest positive subnormal.
const ETINY: i32 = -101;
/// The integer the shared `exp` overflow gate does not catch:
/// `97 · ln 10 ≈ 223.3` stays inside the format's 224 limit while
/// `10^97` is past `MAX`. Reached here as `compound(9, 6145)`, the
...
fn n_zero_is_one_for_the_whole_domain_and_quiet_nan() {
    for rm in ALL {
        for literal in ["-1", "-0.5", "-0", "0", "1E-101", "1", "9.5", "9.999E+96"] {
            let (r, st) = parse(literal).compound(0, rm);
            assert!(
                equal(r, Decimal32::ONE),
                "compound({literal}, 0) at {rm:?}: got {r}, want 1"
            );
            assert_eq!(st, Status::OK, "compound({literal}, 0) at {rm:?}: flags");
        }
        // +∞ ≥ −1, so it takes the 1 row too.
        let (r, st) = Decimal32::INFINITY.compound(0, rm);
        assert!(equal(r, Decimal32::ONE), "compound(+∞, 0) at {rm:?}: {r}");
        assert_eq!(st, Status::OK);
        // The quiet-NaN carve-out.
        let (r, st) = Decimal32::NAN.compound(0, rm);
        assert!(equal(r, Decimal32::ONE), "compound(qNaN, 0) at {rm:?}: {r}");
...
fn below_negative_one_is_invalid_at_every_n() {
    for rm in ALL {
        for n in [i32::MIN, -7, -1, 0, 1, 7, i32::MAX] {
            ...
            // `Decimal32` gets to the domain edge from below.
            for literal in ["-1.000001", "-2", "-1E+96"] {
            ...
            let (r, st) = Decimal32::NEG_INFINITY.compound(n, rm);
...
fn positive_infinity_follows_the_sign_of_n() {
    for rm in ALL {
        for n in [1, 2, i32::MAX] {
            let (r, st) = Decimal32::INFINITY.compound(n, rm);
            assert!(
                r.is_infinite() && !r.is_sign_negative(),
...
fn below_range_powers_of_ten_underflow_per_mode() {
    ...
    for n in -ETINY + 1..=-ETINY + 40 {
        ...
        assert!(
            equal(r, tiny),
            "compound(−0.9, {n}) at TowardPositive: got {r}, want 1E-101"
        );
...
fn nearest_mode_ties_resolve_by_the_rounders_own_rule() {
    let down = "4.882812E+7";
    let up = "4.882813E+7";
    for (rm, want) in [(NE, down), (NA, up), (TZ, down), (TP, up), (TN, down)] {
        let (r, st) = parse("4").compound(11, rm);
        assert_eq!(text(r), want, "compound(4, 11) at {rm:?}");
        assert_eq!(st, Status::INEXACT, "a tie is inexact in every direction");
    }
    // 5^11 is the ONLY power of five with exactly PRECISION + 1 = 8
    // digits at this precision, so unlike the wider formats there is
    // no second tie: 5^12 carries 9 digits and is decided by an
    // ordinary sticky, not by the tie rule (`NearestAway` keeps it
    // down, which a real tie never does).
    for rm in [NE, NA, TZ, TN] {
        assert_eq!(text(parse("4").compound(12, rm).0), "2.441406E+8");
    }
    assert_eq!(text(parse("4").compound(12, TP).0), "2.441407E+8");

    // The negative-`n` mirror: 2^-11 carries the same coefficient at
    // the mirrored exponent.
    let down_neg = "0.0004882812";
    let up_neg = "0.0004882813";
    for (rm, want) in [
        (NE, down_neg),
        (NA, up_neg),
        (TZ, down_neg),
        ...
        let (r, st) = parse("1").compound(-11, rm);
        assert_eq!(text(r), want, "compound(1, -11) at {rm:?}");
    ...
    let (_, st) = parse("4").compound(13, NE);
    assert_eq!(st, Status::INEXACT);
}
...
fn anchor_band_delivers_by_the_side_theorem() {
    let one = "1.000000";
    let next_up = "1.000001";
    let next_down = "0.9999999";
    // sign(n) = sign(x): the value is strictly above 1.
    for (x, n) in [("1E-15", 1), ("-1E-15", -1), ("1E-20", 7), ("-1E-25", -3)] {
...
fn a_huge_n_leaves_the_anchor_band() {
    let (r, st) = parse("1E-15").compound(i32::MAX, NE);
    assert_eq!(text(r), "1.000002");
    assert_eq!(st, Status::INEXACT);
    let (r, _) = parse("-1E-15").compound(i32::MAX, NE);
    assert_eq!(text(r), "0.9999979");
}
...
/// existed, `compound(1E+30, 1)` at `TowardPositive` delivered
/// `10^200` where the true value `10^200 + 1` is strictly above it and
/// `next_up(10^200)` is owed; the wider powers and the `3E+30` base were the
...
/// The threshold-straddling pair. At `Decimal32` with a one-digit `n`
...
fn the_gate_threshold_does_not_move_the_answer() {
    for n in [1i32, -1] {
        for rm in ALL {
            let (inside, st_in) = parse("1E-12").compound(n, rm);
            let (outside, st_out) = parse("1E-11").compound(n, rm);
            assert_eq!(
                text(inside),
                text(outside),
                "compound(1E-12, {n}) and compound(1E-11, {n}) at {rm:?} \
...
fn n_one_is_the_sum() {
    for literal in ["0.05", "1", "9", "-0.5", "1E-30", "-1E-30", "123.456"] {
        let x = parse(literal);
        let (want, want_st) = Decimal32::ONE.add(x, NE);
