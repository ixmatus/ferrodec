//! Integer-argument, special-value, and flag gate for `Decimal32`'s
...
//! overflow gate and the true overflow boundary (`n = 97` here)
...
/// `Decimal32` significand width: the exact family's `|n|` ceiling.
const PRECISION: u32 = 7;
/// `Decimal32`'s largest exponent: `10^E_MAX` is representable,
/// `10^(E_MAX + 1)` is not. `E_MAX + 1` is also the gap integer
/// between the kernel's overflow gate (which fires above
/// `x ≈ 97.3`) and the true overflow boundary.
const E_MAX: i32 = 96;
...
fn positive_neighbours(n: i32) -> (Decimal32, Decimal32) {
...
/// answers.
fn negative_neighbours() -> (Decimal32, Decimal32) {
    let away = parse("-1");
...
fn coarse_integer_rows_every_mode() {
    for n in [
        50i32, 90, E_MAX, -50, -1000,
        -101, // etiny scale: the result still sits just above −1
        -99_999, -100_000,
    ] {
        check_integer(n);
    }
...
/// The rung-2 misround family witness. At `n = 60` the working value
/// `10^60 ⊖ 1` is exactly the grid point `1e60` at rung 1 (which
/// absorbs the `1` from `n = 50`), and the whole family past
/// `n = 110` is stuck at rung 2 as well, so before the classifier the
/// directed modes were decided by the sign of the kernel's noise.
/// `TowardZero` is the row that pins it: the true value `10^60 − 1`
/// is one unit below `1e60`, so the answer is the 7 nines value at
/// that decade, written out here in full.
#[test]
fn rung2_misround_family_witness_at_60() {
    let x = parse("60");
    let want_toward = parse("9.999999e59");
    let want_away = parse("1e60");

    let (r, st) = x.exp10_m1(TZ);
    assert!(
        equal(r, want_toward),
        "exp10_m1(60) at TowardZero: got {r}, want {want_toward}"
    );
    assert!(st.inexact(), "exp10_m1(60) at TowardZero: INEXACT");
    let (r, _) = x.exp10_m1(TN);
    assert!(
        equal(r, want_toward),
        "exp10_m1(60) at TowardNegative: got {r}, want {want_toward}"
    );
    for rm in [NE, NA, TP] {
        let (r, _) = x.exp10_m1(rm);
        assert!(
            equal(r, want_away),
            "exp10_m1(60) at {rm:?}: got {r}, want {want_away}"
        );
    }
}
...
fn subnormal_results_raise_underflow() {
    let (r, st) = parse("1e-99").exp10_m1(NE);
    assert!(r.is_subnormal(), "exp10_m1(1e-99) is subnormal, got {r}");
    assert!(
        st.underflow() && st.inexact(),
        "exp10_m1(1e-99): want UNDERFLOW | INEXACT"
    );
...
fn inexact_flag_is_honest_in_every_mode() {
    ...
    for literal in [
        "2", "2.000", "5", "7", "0.7e1", "-2", "-2.000", "-7", "-0.7e1",
    ] {
        let x = parse(literal);
        for rm in ALL {
            let (_, st) = x.exp10_m1(rm);
...
fn special_values_every_mode() {
    for rm in ALL {
        let (r, st) = Decimal32::ZERO.exp10_m1(rm);
        assert!(
        ...
        let (r, st) = Decimal32::NEG_ZERO.exp10_m1(rm);
        assert!(
            r.is_zero() && r.is_sign_negative(),
            "exp10_m1(-0) at {rm:?}"
        );
        ...
        let (r, st) = Decimal32::NEG_INFINITY.exp10_m1(rm);
        assert!(
        ...
        let (r, st) = Decimal32::INFINITY.exp10_m1(rm);
        assert!(
            r.is_infinite() && !r.is_sign_negative(),
            "exp10_m1(+inf) at {rm:?}"
        );
