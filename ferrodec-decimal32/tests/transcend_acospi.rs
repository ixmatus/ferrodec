...
//! `Decimal32::acos_pi` (IEEE 754-2019 §9.2 `acosPi`; ADR-0061
...
//! anchor gate is `adj(x) ≤ -(P + 3) = -10` and the neighbours of
//! `1/2` are `0.4999999` and `0.5000001`.
//!
...
/// digits at quantum `10^-P`, so the neighbours sit `10^-7` away and
...
fn specials_per_section_9_2_1() {
    for rm in ALL {
        ...
        let (r, st) = Decimal32::NAN.acos_pi(rm);
        assert!(r.is_nan() && st.is_ok(), "acosPi(NaN) [{rm:?}]");
        let (r, st) = Decimal32::SIGNALING_NAN.acos_pi(rm);
...
/// The non terminating rational rows at `P = 7`. `1/3 = 0.333…` has
...
fn one_third_and_two_thirds_rows() {
    const THIRD_DOWN: &str = "0.3333333";
    const THIRD_UP: &str = "0.3333334";
    for (rm, want) in [
        (NE, THIRD_DOWN),
        (NA, THIRD_DOWN),
        (TZ, THIRD_DOWN),
    ...
    const TWO_THIRDS_DOWN: &str = "0.6666666";
    const TWO_THIRDS_UP: &str = "0.6666667";
    for (rm, want) in [
        (NE, TWO_THIRDS_UP),
        (NA, TWO_THIRDS_UP),
        (TZ, TWO_THIRDS_DOWN),
    ...
    let third_long = format!("0.{}", "3".repeat(30));
    let two_thirds_long = format!("0.{}", "6".repeat(30));
    for rm in ALL {
...
fn range_and_monotonicity() {
    let mut previous: Option<Decimal32> = None;
    for step in 0..=20i32 {
