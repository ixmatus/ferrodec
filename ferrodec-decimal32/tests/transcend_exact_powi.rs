...
//! for `Decimal32`'s `powi` (IEEE 754-2019 §9.2 `pown`; ADR-0059
...
const EMAX: i32 = 96;
/// Smallest decimal exponent of a representable `Decimal32`.
const ETINY: i32 = -101;
...
fn row_n_zero_is_one_unless_the_base_signals() {
    for x in [
        ...
        for rm in ALL {
            ...
            assert!(equal(r, Decimal32::ONE), "powi({x}, 0) at {rm:?} = {r}");
            assert_eq!(st, Status::OK, "powi({x}, 0) at {rm:?}: flags");
        }
    }
    for rm in ALL {
        let (r, st) = Decimal32::SIGNALING_NAN.powi(0, rm);
...
fn row_zero_base_negative_n_divides_by_zero() {
    for n in [-1, -3, -7, -49, i32::MAX.wrapping_neg()] {
        ...
        for (x, want_neg) in [(Decimal32::ZERO, false), (Decimal32::NEG_ZERO, true)] {
            for rm in ALL {
    ...
    for n in [-2, -4, -8, -112, i32::MIN] {
        ...
        for x in [Decimal32::ZERO, Decimal32::NEG_ZERO] {
            for rm in ALL {
                let (r, st) = x.powi(n, rm);
                assert!(
...
fn row_infinite_base() {
    for n in [1, 3, 49, i32::MAX] {
        for rm in ALL {
            let (r, st) = Decimal32::INFINITY.powi(n, rm);
            assert!(r.is_infinite() && !r.is_sign_negative(), "powi(+inf, {n})");
            assert_eq!(st, Status::OK);
            let (r, st) = Decimal32::NEG_INFINITY.powi(n, rm);
            assert!(r.is_infinite() && r.is_sign_negative(), "powi(-inf, {n})");
            assert_eq!(st, Status::OK);
        }
    }
    for n in [2, 4, 112] {
        for rm in ALL {
            let (r, _) = Decimal32::INFINITY.powi(n, rm);
            assert!(r.is_infinite() && !r.is_sign_negative(), "powi(+inf, {n})");
            let (r, _) = Decimal32::NEG_INFINITY.powi(n, rm);
            assert!(
                r.is_infinite() && !r.is_sign_negative(),
...
/// `2^23` is exactly 7 digits: the widest exact power of two the
/// format holds, delivered clean in every direction. `2^24` needs 8
/// and is the first inexact one, where the dropped digit is a 6 —
/// above the midpoint, so both nearest modes step up with
/// `TowardPositive` and only the two toward-zero directions keep the
/// truncation.
#[test]
fn two_to_the_23_is_exact_and_24_is_not() {
    let want = parse("8388608");
    for rm in ALL {
        let (r, st) = parse("2").powi(23, rm);
        assert!(equal(r, want), "powi(2, 23) at {rm:?} = {r}");
        assert_eq!(st, Status::OK, "powi(2, 23) at {rm:?}: flags");
    }
    let down = parse("1.677721E+7");
    let up = parse("1.677722E+7");
    for (rm, w) in [(NE, up), (NA, up), (TZ, down), (TN, down), (TP, up)] {
        let (r, st) = parse("2").powi(24, rm);
        assert!(equal(r, w), "powi(2, 24) at {rm:?} = {r}, want {w}");
        assert!(st.inexact(), "powi(2, 24) at {rm:?}: want INEXACT");
        assert!(!st.overflow(), "powi(2, 24) at {rm:?}: no overflow");
    }
}

/// The real tie. `5^11` has exactly 8 digits and ends in 5, so the
...
/// the truncation.
#[test]
fn the_tie_at_five_to_the_11_resolves_by_mode() {
    let down = parse("4.882812E+7");
    let up = parse("4.882813E+7");
    for (rm, w, label) in [
        ...
        let (r, st) = parse("5").powi(11, rm);
        assert!(equal(r, w), "powi(5, 11) at {rm:?} = {r}: {label}");
        assert!(st.inexact(), "powi(5, 11) at {rm:?}: want INEXACT");
    }
    // The negative-exponent mirror: 2^-11 = 5^11 · 10^-11, the same
    // 8-digit coefficient one decade family down.
    for (rm, w) in [(NE, "4.882812E-4"), (NA, "4.882813E-4")] {
        let (r, st) = parse("2").powi(-11, rm);
        assert!(equal(r, parse(w)), "powi(2, -11) at {rm:?} = {r}");
        assert!(st.inexact(), "powi(2, -11) at {rm:?}: want INEXACT");
    }
    ...
    // magnitude.
    let (r, _) = parse("-5").powi(11, TN);
    assert!(equal(r, up.neg()), "powi(-5, 11) at TowardNegative = {r}");
    let (r, _) = parse("-5").powi(11, TP);
    assert!(equal(r, down.neg()), "powi(-5, 11) at TowardPositive = {r}");
}
...
fn n_plus_minus_one_is_identity_and_reciprocal() {
    ...
    for literal in ["3", "7", "-3", "1.7", "9.87654321"] {
        ...
        for rm in ALL {
            let (r, st) = x.powi(-1, rm);
            let (d, ds) = Decimal32::ONE.div(x, rm);
            assert!(
                equal(r, d),
...
fn above_range_overflows_per_mode() {
    let mut cases: Vec<(Decimal32, i32, &str)> = Vec::new();
    for n in [
        EMAX + 1,
        EMAX + 2,
        10_000,
    ...
    cases.push((parse("1e50"), 6, "1e50"));
    cases.push((parse("1.7e50"), 6, "1.7e50"));
    cases.push((parse("1.7e50"), 7, "1.7e50"));
    cases.push((parse("1e50"), 2000, "1e50"));
...
fn below_range_underflows_per_mode() {
    ...
    cases.push((parse("1e-50"), 6, "1e-50"));
    cases.push((parse("1.7e-50"), 6, "1.7e-50"));
    cases.push((parse("1.7e-50"), 7, "1.7e-50"));
    cases.push((parse("1e50"), -2000, "1e50"));
    for (x, n, label) in cases {
...
fn exact_results_in_the_subnormal_band_are_clean() {
    ...
    // 5^3 · 10^-102 needs a quantum one below `etiny`: inexact, and
    // tiny, so both flags are owed.
    for (base, n) in [("5e-34", 3), ("3e-51", 2)] {
...
fn the_powering_arms_reciprocal_survives_out_of_range_accumulators() {
    for (base, n) in [("1.7e50", -6), ("1.7e40", -5), ("9.9e96", -6)] {
        for rm in [NE, NA, TZ, TN] {
    ...
    for (base, n) in [("1.7e-50", -6), ("1.7e-40", -5), ("9e-101", -6)] {
        for rm in [NE, NA, TP] {
            let (r, st) = parse(base).powi(n, rm);
            assert!(
                r.is_infinite() && !r.is_sign_negative(),
    ...
    for rm in [NE, NA, TN] {
        let (r, st) = parse("-1.7e-50").powi(-5, rm);
        assert!(
            r.is_infinite() && r.is_sign_negative(),
            "powi(-1.7e-50, -5) at {rm:?} = {r}, want -inf"
        );
        assert!(st.overflow() && st.inexact(), "flags at {rm:?}: {st:?}");
    }
    for rm in [TZ, TP] {
        let (r, _) = parse("-1.7e-50").powi(-5, rm);
        assert!(
            equal(r, Decimal32::MAX.neg()),
            "powi(-1.7e-50, -5) at {rm:?} = {r}, want -MAX"
        );
    }
}
...
fn the_arm_seam_agrees_with_pow() {
    let bases = [
        "1.7", "-1.7", "3", "-3", "0.7", "9.876543", "1.000001", "0.999999", "1234.567", "-0.001",
        "7e10", "3e-10", "2.5", "1e-4",
    ];
    for base in bases {
        let x = parse(base);
        for n in [-8i32, -7, -6, -5, -2, -1, 1, 2, 5, 6, 7, 8] {
            let y = parse(&format!("{n}"));
            for rm in ALL {
...
fn the_powering_arm_composes() {
    for base in ["1.5", "-1.5", "2", "0.2", "3", "-0.5"] {
        let x = parse(base);
        ...
        let (recip, _) = Decimal32::ONE.div(pos, NE);
        assert!(
            equal(neg, recip),
...
fn unit_bases_at_every_exponent() {
    for n in [1, -1, 2, -2, 7, -7, 99_999, -99_999, i32::MAX, i32::MIN] {
        for rm in ALL {
            let (r, st) = Decimal32::ONE.powi(n, rm);
            assert!(equal(r, Decimal32::ONE), "powi(1, {n}) at {rm:?} = {r}");
            assert_eq!(st, Status::OK, "powi(1, {n}): flags");
            let (r, st) = Decimal32::NEG_ONE.powi(n, rm);
            let want = if n % 2 == 0 {
                Decimal32::ONE
            } else {
                Decimal32::NEG_ONE
