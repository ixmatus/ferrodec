//! Exact-result, tie, and special-value gate for `Decimal32::exp2_m1`
//! (IEEE 754-2019 §9.2 `exp2m1`; ADR-0059 Track D), the sibling mirror
//! of the root crate's `tests/transcend_exact_exp2m1.rs`.
//!
...
//! file is the claim's exhaustive witness at 7 digits: every `n` in
//! `1..=23` and every `m` in `1..=7`, in all five rounding directions,
//! delivered exactly with status `OK` and no `INEXACT` (§7.5 forbids
//! it on an exact result). The narrowest format holds the tightest
//! family edges, `n = 23` (`2^23 − 1 = 8388607`) and `m = 7`
//! (`10^7 − 5^7 = 9921875`), both exactly 7 digits.
//!
...
//! `n = 24` is the single multiple of four whose value spans 8 digits;
//! `10^m − 5^m` always ends in 5, so `m = 8` is a midpoint too. The
//! corpus generator excludes both inputs from every scan mode, because
//! a certified Arb ball around an exact midpoint never becomes
//! decisive, so these literal assertions are the ties' only witnesses.
//! Each delivery is asserted twice: against the value the derivation
//! predicts, spelled out in full, and against the format's own decimal
//! parser applied to the exact midpoint string, which rounds it under
//! the same mode through a path that shares no code with the kernel.
...
/// The format's exact-family ceilings: `2^23 − 1` is exactly 7 digits,
/// and `10^7 − 5^7` is exactly 7 digits.
const N_MAX: u32 = 23;
const M_MAX: u32 = 7;

/// The two tie inputs at this format: `2^24 − 1` is the one 8-digit
/// value with `4 | n`, and `m = PRECISION + 1 = 8` is always one.
const TIE_POS: u32 = 24;
const TIE_NEG: u32 = 8;

/// The exact midpoint `2^24 − 1`, spelled out.
const TIE_POS_MID: &str = "16777215";
/// Its neighbour of larger magnitude (`1677722E+1`, the even one).
const TIE_POS_UP: &str = "16777220";
/// Its neighbour of smaller magnitude (`1677721E+1`).
const TIE_POS_DOWN: &str = "16777210";

/// The exact midpoint `2^−8 − 1`, spelled out.
const TIE_NEG_MID: &str = "-0.99609375";
/// Its neighbour of larger magnitude (the even one).
const TIE_NEG_UP: &str = "-0.9960938";
/// Its neighbour of smaller magnitude.
const TIE_NEG_DOWN: &str = "-0.9960937";

/// The representable value one step toward zero from `−1`.
const MINUS_ONE_NEIGHBOUR: &str = "-0.9999999";
...
/// Every `exp2m1(n) = 2^n − 1` in reach of 7 digits, all five modes.
#[test]
...
/// Every `exp2m1(−m) = −(10^m − 5^m)·10^−m` in reach of 7 digits, all
/// five modes.
#[test]
...
fn exact_family_matches_the_independent_witness() {
    for n in 1..=N_MAX {
        ...
        let (sum, st) = r.add(Decimal32::ONE, NE);
...
/// The two `Decimal32` rows of the derivation's tie table, literal,
/// every mode. §7.5: a tie delivery drops a nonzero digit in every
/// mode, so `INEXACT` is raised on every one of them.
#[test]
...
/// The negative-side `Decimal32` tie row; see
...
fn window_edges_and_tie_neighbours() {
    for rm in ALL {
        assert_exact(
            int_input(i32::try_from(N_MAX).unwrap()).exp2_m1(rm),
            parse("8388607"),
            &format!("exp2_m1({N_MAX}) [{rm:?}]"),
        );
    }
...
fn minus_one_band_spot_rows() {
    ...
    for label in ["-400", "-1000", "-1e6", "-1e50", "-9.999999e96"] {
        let x = parse(label);
        for (rm, want) in [
            (NE, Decimal32::NEG_ONE),
            (NA, Decimal32::NEG_ONE),
            (TZ, neighbour),
            (TP, neighbour),
            (TN, Decimal32::NEG_ONE),
        ] {
            let (r, st) = x.exp2_m1(rm);
            assert!(
                eq(r, want),
    ...
    for label in ["-160", "-170"] {
        ...
        assert!(eq(r, Decimal32::NEG_ONE), "exp2_m1({label}) = {r}");
...
/// The overflow band: past `x ≈ +324` the §7.4 disposition applies per
/// direction.
#[test]
fn overflow_band_spot_rows() {
    for label in ["500", "1e6", "1e50", "9.999999e96"] {
        let x = parse(label);
        for rm in ALL {
            let (r, st) = x.exp2_m1(rm);
            assert!(
...
fn specials_per_section_9_2_1() {
    for rm in ALL {
        let (r, st) = Decimal32::ZERO.exp2_m1(rm);
        assert!(r.is_zero() && !r.is_sign_negative(), "exp2_m1(+0) = {r}");
        assert_eq!(st, Status::OK, "exp2_m1(+0) status {st:?}");
        let (r, st) = Decimal32::NEG_ZERO.exp2_m1(rm);
        assert!(r.is_zero() && r.is_sign_negative(), "exp2_m1(-0) = {r}");
        assert_eq!(st, Status::OK, "exp2_m1(-0) status {st:?}");

        let (r, st) = Decimal32::NEG_INFINITY.exp2_m1(rm);
        assert!(eq(r, Decimal32::NEG_ONE), "exp2_m1(-inf) = {r}");
        assert_eq!(st, Status::OK, "exp2_m1(-inf) status {st:?}");

        let (r, st) = Decimal32::INFINITY.exp2_m1(rm);
        assert!(
            r.is_infinite() && !r.is_sign_negative(),
            "exp2_m1(+inf) = {r}"
        );
        ...
        let (r, st) = Decimal32::NAN.exp2_m1(rm);
        assert!(r.is_nan() && st.is_ok(), "exp2_m1(NaN) = {r} {st:?}");
        let (r, st) = Decimal32::SIGNALING_NAN.exp2_m1(rm);
...
fn flag_honesty_across_modes() {
    for label in [
        "0.1", "2.5", "-0.3", "-1.5", "1e50", "1e-50", "1e-95", "-1e-95", "1234.567",
    ] {
        let x = parse(label);
        for rm in ALL {
...
/// must decline, and the kernel must then raise `INEXACT`.
#[test]
fn near_misses_are_declined() {
    for label in [
        // Non-integers either side of exact integer inputs (7 digits
...
/// grid point. `exp2m1` carries no x-anchor seam by design.
#[test]
fn tiny_inputs_deliver_without_an_x_anchor_seam() {
    for label in [
        "1e-40", "-1e-40", "1e-80", "-1e-80", "1e-95", "-1e-95", "1e-101", "-1e-101",
    ] {
        let x = parse(label);
        for rm in ALL {
            let (r, st) = x.exp2_m1(rm);
        ...
        // is below 1. Away from the subnormal floor only, where the
        // format quantum rather than the slope decides.
        if !r.is_subnormal() {
            assert!(
