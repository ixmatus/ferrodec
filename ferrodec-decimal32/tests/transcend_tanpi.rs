...
//! `Decimal32::tan_pi` (IEEE 754-2019 §9.2 `tanPi`; ADR-0061 Track D
//! group D4).
//!
...
//! `|tanPi| = |cot(πδ)| ≤ 1/(π|δ|) ≤ 10^7/π ≈ 3.18·10^6` — 90
...
//! below by `10^-7`, so the tightest hug the format can express is
//! `2π·10^-7 ≈ 6.3·10^-7`, which rung 1 resolves with thirty-nine decades
//! to spare. The kernel carries the ADR-0051 arm ADR-0061 specifies,
//! but its gate (`adj(ε) ≤ −11`) sits three decades below that floor
...
/// `tan(54°) = √(5 + 2√5)` at 7 digits.
const TAN_54: &str = "1.376382";
/// `tan(18°)` at 7 digits.
const TAN_18: &str = "0.3249197";
...
fn assert_zero(got: (Decimal32, Status), neg: bool, label: &str) {
    let (r, st) = got;
...
fn specials_per_section_9_2_1() {
    for rm in ALL {
        // tanPi(±0) is ±0.
        assert_zero(
            Decimal32::ZERO.tan_pi(rm),
            false,
            &format!("tanPi(+0) [{rm:?}]"),
        );
        assert_zero(
            Decimal32::NEG_ZERO.tan_pi(rm),
            true,
            &format!("tanPi(-0) [{rm:?}]"),
        );
        ...
        let (r, st) = Decimal32::NAN.tan_pi(rm);
        assert!(r.is_nan() && st.is_ok(), "tanPi(NaN) [{rm:?}] = {r} {st:?}");
        let (r, st) = Decimal32::SIGNALING_NAN.tan_pi(rm);
        assert!(
            r.is_nan() && st.invalid(),
            "tanPi(sNaN) [{rm:?}] = {r} {st:?}"
        );
...
/// neighbours of a pole. `|δ| = 10^-7` is the tightest offset the
/// format can spell beside `0.5`, so this is the largest pole value the
/// operation can ever produce: `1/(π·10^-7) ≈ 3.18·10^6`, finite,
/// with 90 decades of headroom. The sign alternates across the pole.
#[test]
fn poles_cannot_overflow() {
    // `10^7/π`, from `1/π = 0.3183098861837906715…`
    // rounded to this format's 7 digits. `cot(t) = 1/t − t/3 − …`, and
    // the correction is `≈ 10^-7`, seven decades under the last
    // digit, so the cap is `1/t` to every digit the format carries.
    let cap = parse("3.183099E+6");
    // The global ceiling `10^7/π` over every pole neighbourhood, and a
    // floor loose enough to hold for all of them.
    let global_cap = parse("3.184E+6");
    let floor = parse("1E+5");
    // (operand, expected result sign). The offset a pole neighbour can
    // carry depends on the operand's own adjusted exponent: beside
    // `0.5` it is `10^-7`, beside `1.5` only `10^-6`, because the
    ...
    for (label, neg) in cases {
        for rm in ALL {
            ...
            assert!(
                ...
                "tanPi({label}) [{rm:?}] = {r} left the proven 10^7/π cap"
            );
        }
    }
    // The tightest neighbour the format can spell lands on the derived
    // cap `1/(π·10^-7)`: the largest value this operation can produce.
    let (r, _) = parse("0.4999999").tan_pi(NE);
    assert!(
        eq(r, cap),
        "the pole cap drifted: got {r}, want {cap} (= 1/(π·10^-7))"
    );
}
...
/// `2π·10^-7`, which straddles the `5·10^-7` midpoint above 1 and so
...
fn period_one_is_bitwise() {
    // Every operand here keeps a spare digit, so `x + 1` is exact at
    // this width and the comparison reads the reduction rather than the
    // addition.
    for label in ["0.3", "0.1", "0.7", "0.25", "0.75", "0.0001", "0.123456"] {
        let x = parse(label);
...
fn small_arguments_track_the_pi_slope() {
    ...
    for label in ["1E-20", "1E-40", "1E-50", "1E-50"] {
        let x = parse(label);
        for rm in ALL {
            let (r, st) = x.tan_pi(rm);
            assert!(st.inexact(), "tanPi({label}) [{rm:?}] must be INEXACT");
        ...
        assert!(
            scaled.abs().partial_cmp(parse("1E-5")).0 == Some(Ordering::Less),
