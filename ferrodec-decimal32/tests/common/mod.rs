//! Thin `Decimal32` adapter onto the shared, generic faithful-rounding
...
//! `ferrodec-decimal32` therefore declares the weaker **faithful
//! rounding** contract for `exp` / `ln`: the returned value is one of
//! the two representable `Decimal32` values immediately adjacent to the
...
//! function-approximation error `~10^-70`, far below one `Decimal32`
//! ULP of `~10^-7`). Faithfulness is then decided structurally using
...
//! directed `ferrodec-decimal32` rounding modes additionally pin
...
//! `FaithfulFormat` trait. The [`D32`] newtype below is the
//! `Decimal32` implementation of that trait (a newtype because the
//! orphan rule forbids implementing a foreign trait for the foreign
//! `Decimal32` directly from this test crate). This is the
//! **direct-tier** shape: `Decimal32`'s own `next_up` / `next_down` /
//! `{:e}` feed the shared astro-float oracle at the Decimal32 boundary
//! (no Decimal64 widen carrier). It is `Decimal32` TIER-1 in
//! structure, but Design-A keeps `ferrodec-decimal32`'s own manifest
//! astro-float-free: `BigFloat` / `Consts` reach this file only through
//! the `ferrodec_test_support::transcend_oracle` re-export, so
//! astro-float compiles transitively inside the test-support crate and
//! never appears in the decimal32 dependency graph. Same-named,
//! concrete `Decimal32`-signature wrappers (`assert_faithful`, `parse`,
//! `MODES`) sit on top so every `tests/property_*.rs` keeps its
//! `use common::{...}` lines and call sites byte-unchanged.

#![allow(dead_code)]

use ferrodec_decimal32::{Decimal32, RoundingMode, Status};
use ferrodec_test_support::transcend_oracle::{
    self, BigFloat, Consts, FaithfulFormat, SpecRounding,
};

/// Newtype carrier so the foreign [`FaithfulFormat`] trait can be
/// implemented for the foreign `Decimal32` from this test crate
...
pub struct D32(pub Decimal32);

impl FaithfulFormat for D32 {
    ...
    /// round-to-nearest-even. Same order as the `Decimal64` adapter.
    ...
    fn parse_nearest_even(s: &str) -> Self {
        D32(Decimal32::parse_str(s, RoundingMode::NearestEven)
            .unwrap()
            .0)
    }
    ...
    fn next_up(self) -> Self {
        D32(self.0.next_up().0)
    }

    fn next_down(self) -> Self {
        D32(self.0.next_down().0)
    }
    ...
    /// Map `ferrodec-decimal32`'s concrete `RoundingMode` onto the
    ...
    /// pin a side. Identical grouping to the `Decimal64` adapter.
...
pub const MODES: &[RoundingMode] = <D32 as FaithfulFormat>::MODES;
...
/// reference values. Concrete `Decimal32` signature so consumers stay
/// byte-unchanged.
pub fn parse(s: &str) -> Decimal32 {
    transcend_oracle::parse::<D32>(s).0
}

/// `true` iff `got` is faithfully rounded with respect to the true
/// value `oracle`: no representable `Decimal32` lies strictly between
/// `got` and the true result. Concrete-`Decimal32` wrapper around the
/// generic harness; semantics unchanged.
pub fn within_faithful_ulp(got: Decimal32, oracle: &BigFloat, cc: &mut Consts) -> bool {
    transcend_oracle::within_faithful_ulp(D32(got), oracle, cc)
}

/// `true` iff `got` is on the spec-mandated side of the true value for
/// rounding mode `rm`. Concrete-`Decimal32` wrapper around the generic
...
pub fn faithful_side_ok(
    ...
    transcend_oracle::faithful_side_ok(D32(got), oracle, cc, rm)
}

/// Assert the full faithful-rounding contract for one transcendental
/// evaluation. Concrete-`Decimal32` wrapper around the generic
...
pub fn assert_faithful(
    ...
    transcend_oracle::assert_faithful(D32(got), status, oracle, cc, rm, ctx);
}
...
/// `Decimal32` wrapper around the generic harness; semantics unchanged.
pub fn within_n_ulp_band(got: Decimal32, want: Decimal32, n_ulps: u32, cc: &mut Consts) -> bool {
    transcend_oracle::within_n_ulp_band(D32(got), D32(want), n_ulps, cc)
}
