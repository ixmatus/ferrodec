//! `Decimal32::ln_1p` (IEEE 754-2019 §9.2 `logp1`): special values,
//! flag honesty, the `ln` cross check, the ADR-0051 anchor band, and a
//! property sweep over the whole finite domain `x > −1`. The
//! `Decimal128` mirror is `<workspace>/tests/property_ln_1p.rs`; the
//! two differ only in the format's precision and exponent range.
//!
...
/// gate uses).
fn eq(a: Decimal32, b: Decimal32) -> bool {
...
fn zeros_return_themselves_sign_preserved_and_exception_free() {
    for rm in ALL {
        let (r, s) = Decimal32::ZERO.ln_1p(rm);
        assert!(
        ...
        let (r, s) = Decimal32::NEG_ZERO.ln_1p(rm);
        assert!(
            r.is_zero() && r.is_sign_negative(),
            "logp1(-0) = -0 [{rm:?}]"
        );
...
fn minus_one_is_negative_infinity_with_div_by_zero() {
    for rm in ALL {
        let (r, s) = Decimal32::NEG_ONE.ln_1p(rm);
        assert!(
...
fn below_minus_one_is_invalid_nan() {
    ...
    let just_below = Decimal32::NEG_ONE.next_down().0;
    for rm in ALL {
        for x in [parse("-1.5"), parse("-2"), parse("-1e50"), just_below] {
            let (r, s) = x.ln_1p(rm);
            assert!(r.is_nan(), "logp1({x}) is NaN [{rm:?}]");
            assert!(s.invalid(), "logp1({x}) raises INVALID [{rm:?}]");
        }
        let (r, s) = Decimal32::NEG_INFINITY.ln_1p(rm);
...
fn just_above_minus_one_stays_in_domain() {
    ...
    let x = Decimal32::NEG_ONE.next_up().0;
    for rm in ALL {
        let (r, s) = x.ln_1p(rm);
...
fn positive_infinity_passes_through() {
    for rm in ALL {
        let (r, s) = Decimal32::INFINITY.ln_1p(rm);
        assert!(
            r.is_infinite() && !r.is_sign_negative(),
            "logp1(+inf) [{rm:?}]"
        );
...
/// `t` values whose `t − 1` is exactly representable at 7 digits, so
...
fn agrees_with_ln_wherever_one_plus_x_is_representable() {
    for lit in LN_CROSS_CHECK_T {
        let t = parse(lit);
        let (x, st) = t.sub(Decimal32::ONE, NE);
        assert!(
            !st.inexact(),
            "test premise: {lit} - 1 must be exact, got status {st:?}"
        );
        let (back, _) = Decimal32::ONE.add(x, NE);
        assert!(
            eq(back, t),
...
fn anchor_band_both_signs_follows_the_side_theorem() {
    ...
    check_anchor("1e-40");
    check_anchor("2.718281e-40");
    // Snap band, normal results.
    check_anchor("1e-50");
    check_anchor("1e-90");
    // Snap band, subnormal results (Decimal32 turns subnormal below
    // 1e-95): the UNDERFLOW leg of the assertion above.
    check_anchor("1e-97");
    check_anchor("1e-99");
...
fn in_domain() -> impl Strategy<Value = Decimal32> {
    prop_oneof![
        (1u32..10u32.pow(7), -50i32..=50i32)
            .prop_map(|(coef, exp)| parse(&format!("{coef}e{exp}"))),
        (1u32..10u32.pow(7), 0i32..=50i32).prop_map(|(coef, shift)| {
