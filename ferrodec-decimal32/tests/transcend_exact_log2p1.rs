//! Exact-result and special-value gate for `Decimal32::log2_1p`
...
//! claim's exhaustive witness at 7 digits: every `k` in `1..=23` and
//! every `m` in `1..=7`, in all five rounding directions, delivered
//! exactly with status `OK` and no `INEXACT` (§7.5 forbids it on an
//! exact result). The narrowest format holds the tightest family
//! edges, `k = 23` (`2^23 − 1 = 8388607`) and `m = 7`
...
fn pos_input(k: u32) -> Decimal32 {
    parse(&format!("{}", (1u128 << k) - 1))
}

/// The negative exact family input `x = −(10^m − 5^m)·10^−m`.
fn neg_input(m: u32) -> Decimal32 {
    parse(&format!("-{}e-{m}", 10u128.pow(m) - 5u128.pow(m)))
}

fn assert_exact_int(got: (Decimal32, Status), want: i32, label: &str) {
    let (r, st) = got;
...
/// Every `x = 2^k − 1` in reach of 7 digits, all five modes.
#[test]
...
/// Every `x = −(10^m − 5^m)·10^−m` in reach of 7 digits, all five
...
fn exact_family_matches_the_independent_witness() {
    for k in 1..=K_MAX {
        ...
        let (sum, st) = Decimal32::ONE.add(x, NE);
...
fn specials_per_section_9_2_1() {
    for rm in ALL {
        let (r, st) = Decimal32::ZERO.log2_1p(rm);
        assert!(r.is_zero() && !r.is_sign_negative(), "log2_1p(+0) = {r}");
        assert_eq!(st, Status::OK, "log2_1p(+0) status {st:?}");
        let (r, st) = Decimal32::NEG_ZERO.log2_1p(rm);
        assert!(r.is_zero() && r.is_sign_negative(), "log2_1p(-0) = {r}");
        assert_eq!(st, Status::OK, "log2_1p(-0) status {st:?}");

        let (r, st) = Decimal32::NEG_ONE.log2_1p(rm);
        ...
        // The representable neighbour just past −1 (7 digits, so the
        // literal survives the parse intact).
        let just_past = parse(&format!("-1.{}1", "0".repeat(5)));
        let (r, st) = just_past.log2_1p(rm);
        assert!(
            r.is_nan() && st.invalid(),
            "log2_1p(just below -1) = {r} {st:?}"
        );
        let (r, st) = Decimal32::NEG_INFINITY.log2_1p(rm);
        assert!(r.is_nan() && st.invalid(), "log2_1p(-inf) = {r} {st:?}");

        let (r, st) = Decimal32::INFINITY.log2_1p(rm);
        assert!(
        ...
        let (r, st) = Decimal32::NAN.log2_1p(rm);
        assert!(r.is_nan() && st.is_ok(), "log2_1p(NaN) = {r} {st:?}");
        let (r, st) = Decimal32::SIGNALING_NAN.log2_1p(rm);
...
fn near_misses_are_declined() {
    ...
    // The family ceiling from the other side: `2^23` is itself
    // representable at 7 digits, but `1 + 2^23` is no power of two, so
    // the classifier declines and the kernel raises INEXACT.
