...
//! Decimal128 to Decimal32. Extraction to a shared `ferrodec-ieee`
...
///
/// Each value of [`Decimal32`](crate::Decimal32) belongs to exactly one
/// variant. Use `Decimal32::ieee_class` (added in a subsequent commit)
...
/// a NaN is observable through `Decimal32::is_sign_negative` but does
...
/// `Decimal32::classify`, which returns [`core::num::FpCategory`] (five
/// variants).
...
pub enum IeeeClass {
    ...
    /// `Decimal32::MIN_POSITIVE_NORMAL` (`10⁻⁹⁵`).
    NegativeNormal,
    /// Negative finite value with magnitude strictly below
    /// `Decimal32::MIN_POSITIVE_NORMAL` but strictly above zero.
    NegativeSubnormal,
    /// `−0`. Distinct from [`IeeeClass::PositiveZero`] under
    /// `Decimal32::total_cmp` but equal under
    /// `Decimal32::partial_cmp`.
    NegativeZero,
    /// `+0`. See [`IeeeClass::NegativeZero`] for the comparison
    /// semantics.
    PositiveZero,
    /// Positive finite value strictly below
    /// `Decimal32::MIN_POSITIVE_NORMAL` and strictly above zero.
    PositiveSubnormal,
