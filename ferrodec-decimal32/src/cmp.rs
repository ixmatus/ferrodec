...
impl Decimal32 {
    ...
    ///
    /// Identical to [`Decimal32::partial_cmp`] except that *every* NaN
...
#[inline]
fn quieten_snan(x: Decimal32) -> Decimal32 {
...
mod tests {
    ...
    fn compare_signaling_quiet_nan_raises_invalid() {
        ...
        for (a, b) in [
            (Decimal32::NAN, one),
            (one, Decimal32::NAN),
            (Decimal32::SIGNALING_NAN, one),
            (Decimal32::NAN, Decimal32::NAN),
        ] {
    ...
    fn compare_signaling_matches_partial_cmp_on_non_nan() {
        ...
        for (a, b) in [
            ...
            (two, one),
            (one, one),
            (Decimal32::ZERO, Decimal32::NEG_ZERO),
            (Decimal32::NEG_INFINITY, Decimal32::INFINITY),
        ] {
    ...
    fn min_max_snan_preserves_payload() {
        // IEEE 754-2019 §6.2.3: the result NaN from an sNaN operand
        // must carry that operand's payload.
        use crate::bid::{pack_signaling_nan, T_MASK};
        let payload: u32 = 0x1F_FFF;
        let snan_pos = Decimal32::from_bits(pack_signaling_nan(false, payload));
        let snan_neg = Decimal32::from_bits(pack_signaling_nan(true, payload));

        let (r, s) = from_int(1, 0).min(snan_pos);
        assert!(r.is_quiet_nan());
        assert!(s.invalid());
        assert_eq!(r.to_bits() & T_MASK, payload);
        assert!(!Decimal32::from_bits(r.to_bits()).is_sign_negative());
        ...
        // First-sNaN wins.
        let lhs = Decimal32::from_bits(pack_signaling_nan(false, 0xAAA));
        let rhs = Decimal32::from_bits(pack_signaling_nan(false, 0xBBB));
        let (r, s) = lhs.min(rhs);
        assert!(s.invalid());
