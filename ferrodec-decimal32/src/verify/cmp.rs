...
/// decimal128 was mirrored to decimal32 in the same slice. This
...
/// (Decimal32 has a 20-bit payload field). Symbolic over a 4-bit
...
fn min_max_snan_preserves_payload() {
    ...
    let payload = u32::from(payload4);
    let snan = Decimal32::from_bits(crate::bid::pack_signaling_nan(sign, payload));

    let (r_min, _) = Decimal32::ONE.min(snan);
    assert!(r_min.is_quiet_nan());
    assert!(r_min.to_bits() & crate::bid::T_MASK == payload);

    let (r_max, _) = Decimal32::ONE.max(snan);
