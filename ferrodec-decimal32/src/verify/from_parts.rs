//! Kani harnesses for [`Decimal32::from_parts`], the inverse of
//! [`Decimal32::decode`].
//!
//! `from_parts` composes the `const` `Coefficient` and `BiasedExp`
//! constructors with `bid::pack_finite`. It forms a bijection with
//! `decode` on canonical finite values, proved here in both directions.
//! Because it is a pure encode (no arithmetic), the full `(bool, u32, i16)`
//! domain is tractable, so this harness is symbolic rather than restricted
//! to the special-case operand set used for the arithmetic proofs.
...
fn from_parts_in_range_roundtrips() {
    ...
    let coefficient: u32 = kani::any();
    let exponent: i16 = kani::any();
    let parts = Decimal32Parts {
        negative,
        coefficient,
        exponent,
    };
    ...
    match Decimal32::from_parts(parts) {
        Some(d) => {
            assert!(in_range);
...
fn decode_then_from_parts_bit_identity() {
    ...
    let r = Decimal32::from_parts(p).expect("decoded canonical parts re-encode");
...
fn from_parts_out_of_range_is_none() {
    ...
    assert!(Decimal32::from_parts(parts).is_none());
}
