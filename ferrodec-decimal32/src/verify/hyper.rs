//! Kani harnesses for `Decimal32::{sinh, cosh, tanh, asinh, acosh,
//! atanh}`.
//!
...
fn cosh_special_resolves_and_even() {
    ...
    assert!(rz.to_bits() == Decimal32::ONE.to_bits());

    let (ri, _) = Decimal32::NEG_INFINITY
        .cosh_special_only_for_kani()
...
fn tanh_special_resolves_and_signs() {
    ...
    let (zi, ii, one) = if neg {
        ...
        (Decimal32::ZERO, Decimal32::INFINITY, Decimal32::ONE)
    };
    let (rz, _) = zi
...
fn atanh_special_resolves_and_signs() {
    ...
    assert!(Decimal32::ONE.atanh_special_only_for_kani().is_none());
}
