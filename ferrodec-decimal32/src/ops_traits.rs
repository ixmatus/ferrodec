//! `core::ops` trait implementations for [`Decimal32`] (gated on the
//! `ops` feature).
//!
...
//!
//! `%` routes to [`Decimal32::rem_trunc`] (the GDA truncated
...
impl Rem for Decimal32 {
    type Output = Self;
    /// GDA / C99 `fmod` truncated remainder. Exact, no rounding mode.
    /// Routes to [`Decimal32::rem_trunc`]; the parent format routes
