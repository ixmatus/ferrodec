//! Decimal-digit decomposition for the `Decimal32` coefficient.
//!
...
//! `Decimal32` coefficient fits in `u32`, so the digit buffer is
//! `[u8; PRECISION]` with `PRECISION = 7`.

/// Decimal32 coefficient precision, in base-10 digits.
...
/// which is unreachable for a well-formed BID-32 coefficient.
#[allow(dead_code)] // Wired by ADR-0031 op kernels.
pub(crate) const fn coefficient_to_digits(mut c: u32, out: &mut [u8; PRECISION]) -> usize {
    let mut i = 0usize;
    loop {
        let d = (c % 10) as u8;
        debug_assert!(d <= 9);
...
/// Reconstructs a `u32` coefficient from a digit array (least
/// significant digit at index 0). Bytes outside `0..=9` panic in
/// debug builds.
///
/// The caller is responsible for ensuring the reconstructed value
/// fits in `u32`. The longest valid input has length [`PRECISION`]
/// and bounds the result by `10^PRECISION - 1 = 9_999_999`, well
/// below `u32::MAX`.
#[allow(dead_code)] // Wired by ADR-0031 op kernels.
pub(crate) const fn digits_to_coefficient(digits: &[u8]) -> u32 {
    let mut c: u32 = 0;
...
mod tests {
    ...
    proptest! {
        ...
        #[test]
        fn round_trip(c in 0u32..=10u32.pow(7) - 1) {
            let mut buf = [0u8; PRECISION];
            let count = coefficient_to_digits(c, &mut buf);
            prop_assert!(count >= 1);
        ...
        #[test]
        fn count_agrees_with_decimal_digit_count(c in 0u32..=10u32.pow(7) - 1) {
            let mut buf = [0u8; PRECISION];
            let count = coefficient_to_digits(c, &mut buf);
        ...
        #[test]
        fn digits_in_range(c in 0u32..=10u32.pow(7) - 1) {
            let mut buf = [0u8; PRECISION];
