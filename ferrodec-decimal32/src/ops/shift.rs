//! General Decimal Arithmetic `shift` on `Decimal32`. Same shape as
...
pub(crate) fn validate_shift_rhs(rhs: Decimal32) -> Option<i32> {
...
pub(crate) fn digit_shift(lhs: Decimal32, rhs: Decimal32, wrap: bool) -> (Decimal32, Status) {
    ...
    if coef == 0 {
        return (
            Decimal32::from_bits(pack_finite(sign, bexp_typed, Coefficient::ZERO)),
            Status::OK,
        );
    }
    ...
    let coef_typed = Coefficient::try_new(new_coef).expect("digits-derived coef fits Decimal32");
    (
...
mod tests {
    ...
    fn signaling_nan_lhs_quiets_and_raises_invalid() {
        let (r, st) = Decimal32::SIGNALING_NAN.shift(d(3, 0));
        assert_eq!(st, Status::INVALID);
        assert!(r.is_nan());
