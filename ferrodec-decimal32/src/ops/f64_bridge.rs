...
//!
//! Every `f64`-routed Decimal32 kernel (sin / cos / tan / asin /
...
//! the f64 back to `Decimal32` and tag `INEXACT`. The conversion +
//! tagging step is what this module factors out.
//!
//! Both `f64_unary` (Decimal32 in) and `f64_unary_via_value` (f64
//! in — for callers that already extracted `to_f64()` to check
//! domain before dispatch) cover the same logical operation.
//! `from_f64` returns `(Decimal32, Status)`; we patch the status to
...
//!   the converted Decimal32 is non-zero (transcendentals are
