//! IEEE 754-2019 §9.2 `rSqrt` for [`Decimal32`]: the reciprocal square
...
//!   `Decimal32` input lands strictly inside the normal range
//!   (`10^50` at the smallest subnormal, `~3.16·10^-49` at `MAX`).
...
impl Decimal32 {
    ...
    /// `rsqrt(0.04) = 5`, `rsqrt(2^22) = 5^11·10^-11` is a real
    /// 8-digit midpoint — so §7.5's ban on `INEXACT` for exact results
    ...
    pub fn rsqrt(self, rm: RoundingMode) -> (Self, Status) {
        ferrodec_transcend::rsqrt::rsqrt_kernel::<Decimal32>(self, rm)
    }
}
...
mod tests {
    ...
    #[test]
    fn rsqrt_specials() {
        let (r, st) = Decimal32::INFINITY.rsqrt(RoundingMode::NearestEven);
        assert!(r.is_zero() && !r.is_sign_negative());
        assert!(st.is_ok());

        let (r, st) = Decimal32::ZERO.rsqrt(RoundingMode::NearestEven);
        assert!(r.is_infinite() && !r.is_sign_negative());
        assert!(st.div_by_zero());

        let (r, st) = Decimal32::NEG_ZERO.rsqrt(RoundingMode::NearestEven);
        assert!(r.is_infinite() && r.is_sign_negative());
        assert!(st.div_by_zero());

        let (r, st) = Decimal32::NEG_INFINITY.rsqrt(RoundingMode::NearestEven);
        assert!(r.is_nan() && st.invalid());

        let (r, st) = parse("-4").rsqrt(RoundingMode::NearestEven);
        assert!(r.is_nan() && st.invalid());

        let (r, _) = Decimal32::NAN.rsqrt(RoundingMode::NearestEven);
        assert!(r.is_nan());
        let (r, st) = Decimal32::SIGNALING_NAN.rsqrt(RoundingMode::NearestEven);
        assert!(r.is_nan() && st.invalid());
    }
    ...
    fn rsqrt_two_is_correctly_rounded() {
        ...
        assert!(st.inexact());
        assert!(eq(r, parse("0.7071068")), "rsqrt(2) = {r}");
    }
}
