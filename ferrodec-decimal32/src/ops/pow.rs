...
impl Decimal32 {
    ...
    pub fn powi(self, n: i32, rm: RoundingMode) -> (Self, Status) {
        ferrodec_transcend::pow::powi_kernel::<Decimal32>(self, n, rm)
    }
...
mod tests {
    ...
    #[test]
    fn pow_overflow() {
        // Decimal32's E_MAX is 96; 10^100 exceeds Decimal32's range,
    ...
    fn value_eq(a: Decimal32, b: Decimal32) -> bool {
    ...
    fn pow_exact_results_are_not_inexact() {
        // Rational and small-integer exact powers must not raise INEXACT.
        // Decimal32 E_MAX is 96, so every exact power-of-ten integer case
        // has y ≤ 96 < 256 and is caught by the fast path; the > 256
        // general-path integer case is out of range and is tested on the
        // wider formats.
        let cases = [
            ("4", "0.5", "2"),
            ("9", "0.5", "3"),
