...
//! decTest parser, the directive-aware [`Context`], the per-file
//! expectation guard from ADR-0010, and a generic [`run_suite`]
//! driver. Each sibling's `tests/conformance.rs` is reduced to its
...
//! methods, then call [`run_suite`] from a `#[test]`.
