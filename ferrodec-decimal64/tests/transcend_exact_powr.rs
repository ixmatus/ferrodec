...
//! for `Decimal64`'s `powr` (IEEE 754-2019 §9.2; ADR-0059 Track D D3).
...
fn finite_positive_base_zero_exponent_is_one() {
    for x in ["2", "0.5", "1E+300", "1E-300", "123.456", "9.5"] {
        for z in [Decimal64::ZERO, Decimal64::NEG_ZERO] {
            for rm in ALL {
                let (r, s) = parse(x).powr(z, rm);
                assert!(equal(r, Decimal64::ONE), "powr({x}, ±0) = 1, got {r:?}");
...
fn zero_base_negative_exponent_is_infinity() {
    for z in [Decimal64::ZERO, Decimal64::NEG_ZERO] {
        ...
        for rm in ALL {
            let (r, s) = z.powr(Decimal64::NEG_INFINITY, rm);
            assert!(
                r.is_infinite() && !r.is_sign_negative(),
                "powr(±0, −∞) = +∞"
            );
...
fn zero_base_positive_exponent_is_positive_zero() {
    for z in [Decimal64::ZERO, Decimal64::NEG_ZERO] {
        ...
        let (r, s) = z.powr(Decimal64::INFINITY, NE);
        assert!(r.is_zero() && !r.is_sign_negative(), "powr(±0, +∞) = +0");
        assert_eq!(s, Status::OK);
    }
    // The sign-rule contrast with pow, pinned.
    let (pr, _) = Decimal64::NEG_ZERO.pow(parse("3"), NE);
    assert!(
        pr.is_sign_negative(),
        "pow(−0, 3) = −0 by the odd-integer rule"
    );
    let (qr, _) = Decimal64::NEG_ZERO.powr(parse("3"), NE);
...
fn unit_base_finite_exponent_is_one() {
    for y in ["1", "-1", "0.5", "-3.14", "1E+300", "-1E-300"] {
        for rm in ALL {
            let (r, s) = Decimal64::ONE.powr(parse(y), rm);
            assert!(equal(r, Decimal64::ONE), "powr(1, {y}) = 1, got {r:?}");
...
fn negative_base_is_invalid_for_every_exponent() {
    ...
    for x in ["-2", "-1", "-0.5", "-1E+300"] {
        ...
        for y in [Decimal64::INFINITY, Decimal64::NEG_INFINITY, Decimal64::NAN] {
    ...
    let (r, s) = Decimal64::NEG_INFINITY.powr(parse("2"), NE);
    assert!(r.is_quiet_nan() && s.invalid(), "powr(−∞, 2) is invalid");
    let (r, s) = Decimal64::NEG_ZERO.powr(parse("2"), NE);
...
fn the_three_indeterminate_forms_contrast_with_pow() {
    let zeros = [Decimal64::ZERO, Decimal64::NEG_ZERO];
    let infs = [Decimal64::INFINITY, Decimal64::NEG_INFINITY];
    ...
    for x in zeros {
        for y in zeros {
            ...
            assert!(equal(pr, Decimal64::ONE), "pow(±0, ±0) = 1");
    ...
    for y in zeros {
        for rm in ALL {
            let (r, s) = Decimal64::INFINITY.powr(y, rm);
            assert!(r.is_quiet_nan(), "powr(+∞, ±0) = qNaN, got {r:?}");
            assert!(s.invalid(), "powr(+∞, ±0) signals INVALID");
        }
        let (pr, ps) = Decimal64::INFINITY.pow(y, NE);
        assert!(equal(pr, Decimal64::ONE), "pow(+∞, ±0) = 1");
    ...
    for y in infs {
        for rm in ALL {
            let (r, s) = Decimal64::ONE.powr(y, rm);
            assert!(r.is_quiet_nan(), "powr(+1, ±∞) = qNaN, got {r:?}");
            assert!(s.invalid(), "powr(+1, ±∞) signals INVALID");
        }
        let (pr, ps) = Decimal64::ONE.pow(y, NE);
        assert!(equal(pr, Decimal64::ONE), "pow(+1, ±∞) = 1");
        assert!(!ps.invalid(), "pow(+1, ±∞) raises nothing");
        // pow(−1, ±∞) is also 1; powr refuses it as a negative base.
        let (pr, ps) = Decimal64::NEG_ONE.pow(y, NE);
        assert!(equal(pr, Decimal64::ONE), "pow(−1, ±∞) = 1");
        assert!(!ps.invalid());
        let (qr, qs) = Decimal64::NEG_ONE.powr(y, NE);
...
fn infinite_operand_limits_follow_the_composition() {
    ...
    for y in ["1", "2", "0.5", "1E+300"] {
        let (r, s) = Decimal64::INFINITY.powr(parse(y), NE);
        assert!(
            r.is_infinite() && !r.is_sign_negative(),
            "powr(+∞, {y}) = +∞"
        );
        assert_eq!(s, Status::OK);
        let (r, s) = Decimal64::INFINITY.powr(parse(&format!("-{y}")), NE);
        assert!(r.is_zero() && !r.is_sign_negative(), "powr(+∞, -{y}) = +0");
        assert_eq!(s, Status::OK);
    }
    let (r, _) = Decimal64::INFINITY.powr(Decimal64::INFINITY, NE);
    assert!(r.is_infinite(), "powr(+∞, +∞) = +∞");
    let (r, _) = Decimal64::INFINITY.powr(Decimal64::NEG_INFINITY, NE);
    assert!(r.is_zero(), "powr(+∞, −∞) = +0");

    // powr(x, ±∞) for finite x > 0, x ≠ 1: ln x sets the sign.
    for x in ["2", "10", "1.5", "1E+300"] {
        let (r, s) = parse(x).powr(Decimal64::INFINITY, NE);
        assert!(
            r.is_infinite() && !r.is_sign_negative(),
            "powr({x}>1, +∞) = +∞"
        );
        assert_eq!(s, Status::OK);
        let (r, s) = parse(x).powr(Decimal64::NEG_INFINITY, NE);
...
/// The real `PRECISION + 1` tie: `5^23` is 17 digits ending in 5, one
/// digit past `Decimal64`'s 16, so its true value sits exactly on a
...
///
/// `5^23 = 11920928955078125`; the two neighbours are
/// `1192092895507812E+1` (even) and `1192092895507813E+1`.
#[test]
fn the_five_23_tie_resolves_per_mode() {
    let x = parse("5");
    let y = parse("23");
    let down = parse("1192092895507812E+1");
    let up = parse("1192092895507813E+1");
    let expect = [
    ...
    for (rm, want, why) in expect {
        let (r, s) = x.powr(y, rm);
        assert!(equal(r, want), "powr(5, 23) under {rm:?}: {why}; got {r:?}");
        assert!(s.inexact(), "powr(5, 23) is a tie: INEXACT in every mode");
        // The tie is the classifier's, so powr and pow must agree on it.
        let (pr, ps) = x.pow(y, rm);
        assert!(
            equal(r, pr),
            "powr and pow agree on the 5^23 tie under {rm:?}"
        );
...
fn exact_deliveries_pin_their_quantum() {
    ...
    let cases = [
        ...
        // widest representable cohort, quantum 285.
        ("10", "300", "1.000000000000000E+300"),
...
fn the_ieee_9_2_2_preferred_exponent_conformance() {
    ...
    // shared classifier.
    let (p256, _) = parse("1E+1").pow(parse("256"), NE);
    let (p257, _) = parse("1E+1").pow(parse("257"), NE);
    assert_eq!(
        alloc_string(p256),
        "1E+256",
...
fn alloc_string(d: Decimal64) -> std::string::String {
    std::format!("{d}")
}
