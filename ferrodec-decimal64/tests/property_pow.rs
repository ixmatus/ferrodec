//! Faithful-rounding contract for `Decimal64::pow` vs astro-float,
//! asserted for every IEEE 754 rounding direction (ADR-0021, IEEE
//! 754-2019 §9.2). See `tests/common/mod.rs` for the contract; this
//! is not a `± ULP` tolerance envelope.
//!
...
//! parent uses). The §9.2.1 special-value rule table is exercised
//! separately in `property_pow_specials.rs`; this suite brackets the
//! in-domain finite non-special path (positive base, finite result).
...
#[test]
fn spot_e_to_pi() {
    check_pow("2.718281828459045", "3.141592653589793");
}
#[test]
fn spot_pi_to_e() {
    check_pow("3.141592653589793", "2.718281828459045");
}
#[test]
...
proptest! {
    ...
    /// underflows the Decimal64 range, is outside the faithful-kernel
    ...
    fn pow_integer_y_faithful(
        x_coef in 1u64..=u64::MAX,
        x_exp in -8i32..=8,
        y_int in -256i32..=256,
    ) {
        let xc = x_coef % 10u64.pow(16);
    ...
    fn pow_random_faithful(
        x_coef in 1u64..=u64::MAX,
        x_exp in -8i32..=8,
        y_coef in 1u64..=u64::MAX,
        y_exp in -3i32..=2,
    ) {
        let xc = x_coef % 10u64.pow(16);
        let yc = y_coef % 10u64.pow(16);
