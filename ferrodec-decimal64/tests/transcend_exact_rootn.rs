...
//! `Decimal64`'s `rootn` (IEEE 754-2019 §9.2; ADR-0059 Track D group
...
const ONE_UP: &str = "1.000000000000001";
/// The value just below 1: `1 − 10^−PRECISION`.
const ONE_DOWN: &str = "0.9999999999999999";
/// `2^48`, whose `−2`-th root `2^−24 = 5^24 · 10^−24` carries a
/// 17-digit coefficient ending in 5: a `Decimal64` nearest-mode
/// midpoint (the tie family's only member at this format).
const TIE_INPUT: &str = "281474976710656";
/// The two neighbours the midpoint sits between.
const TIE_DOWN: &str = "5.960464477539062E-8";
const TIE_UP: &str = "5.960464477539063E-8";
/// The hug-at-1 gate at `n = i32::MAX` admits `|x| − 1 ≤ 1.07·10^−8`
/// at this format (`5·10^−(P+2) · (2^31 − 1)`), so `1 + 10^−8` is
/// inside it and `1 + 2·10^−8` is outside.
const STRADDLE_IN: &str = "1.00000001";
const STRADDLE_OUT: &str = "1.00000002";
/// Exponent span for the deterministic `cbrt` differential sweep.
#[cfg(feature = "pow")]
const SWEEP_EXP: i32 = 200;
...
fn zero_negative_order_is_infinity_with_div_by_zero() {
    for rm in ALL {
        for (zero, neg) in [(Decimal64::ZERO, false), (Decimal64::NEG_ZERO, true)] {
...
fn rootn_neg_zero_two_differs_from_sqrt_neg_zero() {
    for rm in ALL {
        let (r, st) = Decimal64::NEG_ZERO.rootn(2, rm);
        assert!(r.is_zero() && !r.is_sign_negative(), "rootn(−0, 2) is +0");
        assert_eq!(st, Status::OK);

        let (s, sst) = Decimal64::NEG_ZERO.sqrt(rm);
        ...
        let (odd, _) = Decimal64::NEG_ZERO.rootn(3, rm);
        assert!(
...
fn infinity_rows() {
    for rm in ALL {
        for n in [1, 2, 3, 100, i32::MAX] {
            let (r, st) = Decimal64::INFINITY.rootn(n, rm);
        ...
        for n in [1, 3, 7, i32::MAX] {
            let (r, st) = Decimal64::NEG_INFINITY.rootn(n, rm);
            assert!(r.is_infinite() && r.is_sign_negative(), "rootn(−∞, {n})");
            assert_eq!(st, Status::OK);
        }
        for n in [-1, -3, -7, -1001] {
...
fn order_minus_one_is_the_reciprocal() {
    for s in [
        ...
        for rm in ALL {
            let (r, st) = x.rootn(-1, rm);
            let (want, want_st) = Decimal64::ONE.div(x, rm);
            assert_eq!(
                r.to_bits(),
                want.to_bits(),
...
#[cfg(feature = "pow")]
fn sweep_inputs(count: usize) -> Vec<Decimal64> {
...
fn one_is_exact_at_every_order() {
    for n in [2, 3, 5, 1_000_000, i32::MAX, -2, -3, -5, i32::MIN] {
        for rm in ALL {
            let (r, st) = Decimal64::ONE.rootn(n, rm);
            assert!(equal(r, Decimal64::ONE), "rootn(1, {n}) at {rm:?} = {r}");
            assert_eq!(st, Status::OK);
        }
        if n % 2 != 0 {
            for rm in ALL {
                let (r, st) = Decimal64::NEG_ONE.rootn(n, rm);
                assert!(equal(r, Decimal64::NEG_ONE), "rootn(−1, {n}) at {rm:?}");
                assert_eq!(st, Status::OK);
            }
        }
    }
...
/// quantum of `0.027` is `10^−17` while the spacing at `0.3` is
/// `10^−16`: the perturbed root sits about `0.37` ULP from `0.3`, so
...
fn negative_order_tie_resolves_by_the_mode() {
    ...
    for (rm, want) in [(NE, down), (NA, up), (TZ, down), (TP, up), (TN, down)] {
        ...
        assert!(
            equal(r, want),
            "rootn(2^48, −2) at {rm:?} = {r}, want {want}"
        );
...
fn quantum_pins() {
    ...
    let x = parse("1E+6");
    let (r, _) = x.rootn(-1, NE);
    let (want, _) = Decimal64::ONE.div(x, NE);
...
/// operands sits ~10^−25 from 1 — nine decades inside the nearest
...
/// `5·10^−18` gate, and the value is a whole quantum off 1 at every
...
fn away_from_one_the_arm_stays_shut() {
    ...
    assert!(st.inexact());
    assert!(!equal(r, Decimal64::ONE), "the value is off the grid point");
    let (cmp, _) = r.partial_cmp(Decimal64::ONE);
