//! S2 deep-margin campaign corpus replay for `Decimal64`
...
//! the `.prov` twins. This gate replays the Decimal64 rows
//! bit-exact with exact per-bucket pins (the priority trio: exp,
//! sin, cos); the Decimal64 gate is the root crate's twin.
...
fn campaign_vectors_correctly_rounded() {
    ...
    frozen::assert_bucket_counts(&vectors, frozen::EXPECTED_BUCKETS_CAMPAIGN_P16);
