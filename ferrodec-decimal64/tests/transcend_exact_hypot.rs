...
//! gate for `Decimal64`'s `hypot` (IEEE 754-2019 §9.2; ADR-0060
//! Track D D3). The `Decimal128` sibling
//! (`tests/transcend_exact_hypot.rs` in the parent crate) carries the
//! full rationale; this file is the same gate at `P = 16`, where the
//! anchor band constant is `δ₀ = ⌈(P + 2)/2⌉ = 9` and the planted
//! nearest-mode tie is a 17-digit hypotenuse ending in 5.
//!
...
/// The largest finite `Decimal64`.
const MAX: &str = "9.999999999999999E+384";
...
fn identical(a: Decimal64, b: Decimal64) -> bool {
    a.to_bits() == b.to_bits()
}
...
fn infinity_beats_quiet_nan_both_orders() {
    for inf in [Decimal64::INFINITY, Decimal64::NEG_INFINITY] {
        for rm in ALL {
            for (p, q) in [(inf, Decimal64::NAN), (Decimal64::NAN, inf)] {
...
fn quiet_nan_with_finite_propagates() {
    for other in [
        ...
        for rm in ALL {
            for (p, q) in [(Decimal64::NAN, other), (other, Decimal64::NAN)] {
                let (r, st) = p.hypot(q, rm);
...
fn exact_pythagorean_family() {
    for (a, b, want) in [
        ...
        ("3E+10", "4E+10", "5E+10"),
        ("3E-10", "4E-10", "5E-10"),
        ("3E+300", "4E+300", "5E+300"),
        ("3E-398", "4E-398", "5E-398"),
        ("30", "0.4E+2", "50"),
        ("3", "0.004E+3", "5"),
        // The widest exact family still inside the format: 15-digit
        // legs, 16-digit hypotenuse.
        ("600000000000000", "800000000000000", "1000000000000000"),
    ] {
        for rm in ALL {
            for (p, q) in [(a, b), (b, a)] {
                let (r, st) = parse(p).hypot(parse(q), rm);
...
fn an_unreachable_preferred_quantum_clamps_without_losing_exactness() {
    for (a, b, want) in [
        ("1E+384", "0E-398", "1.000000000000000E+384"),
        (MAX, "0E-398", MAX),
        ("3E+10", "0E-398", "3.000000000000000E+10"),
        ("1E-398", "0E+369", "1E-398"),
        ("0E+369", "0E-398", "0E-398"),
    ] {
        for rm in ALL {
            for (p, q) in [(a, b), (b, a)] {
                let (r, st) = parse(p).hypot(parse(q), rm);
...
/// The planted nearest-mode tie at `P = 16`: `k = 2000000000000001`
/// (16 digits, odd), legs `3k` and `4k` (16 digits each), hypotenuse
/// `5k = 10000000000000005` — 17 digits ending in 5, i.e. exactly the
/// midpoint between two adjacent `Decimal64` values. Only the exact
/// classifier can resolve a value that *is* a rounding boundary.
#[test]
fn the_planted_tie_resolves_by_the_rounders_own_rule() {
    let a = "6000000000000003";
    let b = "8000000000000004";
    let down = "1.000000000000000E+16";
    let up = "1.000000000000001E+16";
...
/// `hypot(6E+384, 8E+384)` is exactly `1E+385`, one decade past `MAX`:
...
fn an_exact_value_past_the_range_takes_the_overflow_disposition() {
    for (rm, want_inf) in [(NE, true), (NA, true), (TZ, false), (TP, true), (TN, false)] {
        for (p, q) in [("6E+384", "8E+384"), ("8E+384", "6E+384")] {
            let (r, st) = parse(p).hypot(parse(q), rm);
            if want_inf {
...
fn near_sharp_k_squared_plus_one() {
    for k in [
        ...
        let one = parse("1");
        let mut sides = [Decimal64::ZERO; 5];
...
fn near_sharp_k_squared_plus_k() {
    for m in ["3", "10", "1000", "99999999", "1E+8", "1E+100"] {
        let m_d = parse(m);
...
fn the_two_bands_agree_across_the_gate() {
    for w in ["1", "3", "9.999999999999999", "7E+300"] {
        let w_d = parse(w);
        for rm in ALL {
...
fn the_max_edge_splits_the_overflow_disposition() {
    let max = parse(MAX);
    for z in ["1E-300", "1E-398", "1", "1E+200"] {
        let z_d = parse(z);
        for rm in [NE, NA, TZ, TN] {
...
fn the_subnormal_tail() {
    for rm in ALL {
        let (r, st) = parse("3E-398").hypot(parse("4E-398"), rm);
        assert!(identical(r, parse("5E-398")), "@{rm:?}: got {r:e}");
        assert_eq!(st, Status::OK, "exact subnormal @{rm:?}: flags {st:?}");

        let (_, st) = parse("1E-398").hypot(parse("1E-398"), rm);
        assert!(
...
fn symmetry_differential_sweep() {
    ...
    let exps: [i32; 7] = [-398, -300, -16, 0, 17, 100, 350];
    let mut pairs = 0usize;
    for (i, &cx) in coefs.iter().enumerate() {
        for (j, &cy) in coefs.iter().enumerate() {
            let ex = exps[(i + j) % exps.len()];
            let ey = exps[(i + 2 * j + 3) % exps.len()];
            let Ok((x, _)) = Decimal64::parse_str(&format!("{cx}E{ex}"), NE) else {
                continue;
            };
            let Ok((y, _)) = Decimal64::parse_str(&format!("{cy}E{ey}"), NE) else {
                continue;
            };
...
fn oracle_hypot(cx: u128, qx: i32, cy: u128, qy: i32, rm: RoundingMode) -> Rounded {
    ...
    oracle::sqrt(&dec, Format::DECIMAL64, rm)
}

fn matches_oracle(got: Decimal64, want: &Expect) -> bool {
    match want {
        Expect::Nan => false,
        Expect::Infinity { neg } => got.is_infinite() && got.is_sign_negative() == *neg,
        Expect::Finite { neg, coeff, exp } => {
            got.is_finite() && {
                let (n, c, e) = oracle::decode_decimal64(got.to_bits());
                n == *neg && c == *coeff && e == *exp
            }
        }
    }
