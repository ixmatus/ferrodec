...
//! `Decimal64::cos_pi` (IEEE 754-2019 §9.2 `cosPi`; ADR-0061 Track D
//! group D4).
//!
...
//! operand itself and reaches `10^-398`. The kernel gates the channel
//! at `adj(δ) ≤ −⌈(P + 4)/2⌉`, which is `−10` at this format, and
...
//! the last nearest-mode boundary below 1 (`5·10^-17`) once
//! `|δ| < 3.2·10^-9`, so nearest delivers exactly `±1` from there
...
/// `cos(36°) = φ/2` at 16 digits.
const COS_36: &str = "0.8090169943749474";
/// `cos(54°) = sin(36°)` at 16 digits.
const COS_54: &str = "0.5877852522924731";
/// `cos(45°) = √2/2` at 16 digits.
const COS_45: &str = "0.7071067811865475";
/// The largest `Decimal64` strictly below 1.
const JUST_BELOW_ONE: &str = "0.9999999999999999";
...
fn specials_per_section_9_2_1() {
    ...
    for rm in ALL {
        // cosPi(±0) is 1.
        assert_exact(
            Decimal64::ZERO.cos_pi(rm),
            one,
            &format!("cosPi(+0) [{rm:?}]"),
        );
        assert_exact(
            Decimal64::NEG_ZERO.cos_pi(rm),
            one,
            &format!("cosPi(-0) [{rm:?}]"),
        );
        ...
        let (r, st) = Decimal64::NAN.cos_pi(rm);
        assert!(r.is_nan() && st.is_ok(), "cosPi(NaN) [{rm:?}] = {r} {st:?}");
        let (r, st) = Decimal64::SIGNALING_NAN.cos_pi(rm);
        assert!(
            r.is_nan() && st.invalid(),
            "cosPi(sNaN) [{rm:?}] = {r} {st:?}"
        );
...
fn known_values_against_closed_forms() {
    for (label, want) in [
        ...
        ("0.8", "-0.8090169943749474"),
        ("0.7", "-0.5877852522924731"),
        ("0.75", "-0.7071067811865475"),
    ] {
...
///
/// The kernel's channel opens at `adj(δ) ≤ −10`. Nothing observable may
/// change there, so this walks `k` from well above the gate (`10^-9`,
/// ladder territory) down through it (`10^-10` and below) to the
...
fn anchor_band_across_the_gate() {
    ...
    // 10^-9 is one decade ABOVE the gate and still inside the last
    // boundary below 1; 10^-10 is the first gated decade.
    for k in [9i32, 10, 11, 12, 16, 20, 40, 100, 200, 380, 384, 398] {
...
fn anchor_band_beside_the_integers() {
    ...
    let cases: [(&str, bool); 4] = [
        ...
        ("0.999999999999999", false),
        ("2.000000000000001", true),
        ("1.999999999999999", true),
    ];
    for (label, positive) in cases {
        let x = parse(label);
        let sign = |v: Decimal64| if positive { v } else { v.neg() };
        for rm in [NE, NA] {
            let (r, st) = x.cos_pi(rm);
            assert!(
                eq(r, sign(one)),
...
/// `3.2·10^-9` regime boundary and proves the band test above is not
/// vacuously asserting 1 everywhere.
#[test]
fn wide_offsets_do_not_reach_the_anchor() {
    let one = parse("1");
    for label in ["1E-8", "1E-6", "0.00001", "0.0001"] {
        let x = parse(label);
        for rm in ALL {
            let (r, st) = x.cos_pi(rm);
            assert!(st.inexact(), "cosPi({label}) [{rm:?}] must be INEXACT");
    ...
    assert!(eq(r, parse("0.9999999506519784")));
}
