...
//! `Decimal64`'s `exp_m1` (IEEE 754-2019 §9.2 `expm1`; ADR-0059
//! Track D). The sibling mirror of the root crate's
//! `tests/transcend_exact_expm1.rs`, with `Decimal64`'s own
//! thresholds.
//!
...
//! overflow threshold is `Decimal64`'s `887` rather than
...
/// `Decimal64` significand width.
const PRECISION: usize = 16;

/// `Decimal64`'s `exp` overflow gate threshold (`transcend_impl`'s
/// `exp_overflow_limit`); `e^887 ≈ 10^385.2` is past `MAX ≈ 10^385`.
const OVERFLOW_LIMIT: &str = "887";
...
fn zeros_are_the_whole_exact_family() {
    for rm in ALL {
        let (r, st) = Decimal64::ZERO.exp_m1(rm);
        assert!(
        ...
        let (r, st) = Decimal64::NEG_ZERO.exp_m1(rm);
        assert!(
            r.is_zero() && r.is_sign_negative(),
            "expm1(-0) at {rm:?}: got {r}"
        );
...
fn zero_cohort_members_stay_exact() {
    for literal in ["0.000", "-0.000", "0e-300", "-0e100"] {
        let x = parse(literal);
        for rm in ALL {
...
fn inexact_flag_is_honest_in_every_mode() {
    for literal in [
        "1", "-1", "2", "-0.5", "0.25", "10", "-10", "1e-20", "-1e-20", "123.456", "1e-398",
        "-1e-398", "-1000",
    ] {
        let x = parse(literal);
        for rm in ALL {
...
fn minus_one_band_delivers_the_anchor_in_every_mode() {
    ...
    // magnitude.
    for literal in ["-120.5", "-200", "-1000", "-1000000", "-3.5e384"] {
...
fn overflow_follows_section_7_4_on_both_routes() {
    check_overflow("887.5");
    check_overflow("2000");
    check_overflow("1e300");
...
/// must still deliver a finite result. `e^886 ≈ 5.9e384` sits under
/// `MAX ≈ 9.99e384`.
#[test]
fn just_under_the_overflow_gate_stays_finite() {
    ...
    for rm in ALL {
        let (r, st) = x.exp_m1(rm);
        assert!(r.is_finite(), "expm1(886) at {rm:?}: got {r}");
        assert!(!st.overflow(), "expm1(886) at {rm:?}: no OVERFLOW");
        assert!(st.inexact(), "expm1(886) at {rm:?}: expected INEXACT");
    }
}
...
fn anchor_band_both_signs_follows_the_side_theorem() {
    ...
    check_anchor("1e-40");
    check_anchor("1e-50");
    // Normal down to 1e-383, subnormal below it, floor at 1e-398.
    check_anchor("1e-380");
    check_anchor("1e-390");
    check_anchor("1e-398");
}
...
fn special_values_every_mode() {
    for rm in ALL {
        let (r, st) = Decimal64::INFINITY.exp_m1(rm);
        assert!(
            r.is_infinite() && !r.is_sign_negative(),
        ...
        let (r, st) = Decimal64::NEG_INFINITY.exp_m1(rm);
        assert!(
