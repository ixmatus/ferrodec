...
//! `Decimal64::atan_pi` (IEEE 754-2019 §9.2 `atanPi`; ADR-0061
...
//! anchor gate is `adj(x) ≥ P + 2 = 18` and the predecessor of `1/2`
//! is `0.4999999999999999`.
//!
...
const NEG_HALF_DOWN: &str = "-0.4999999999999999";
...
fn specials_per_section_9_2_1() {
    for rm in ALL {
        let (r, st) = Decimal64::ZERO.atan_pi(rm);
        assert!(r.is_zero() && !r.is_sign_negative(), "atanPi(+0) [{rm:?}]");
        assert_eq!(st, Status::OK, "atanPi(+0) [{rm:?}] flags");
        let (r, st) = Decimal64::NEG_ZERO.atan_pi(rm);
        assert!(r.is_zero() && r.is_sign_negative(), "atanPi(-0) [{rm:?}]");
        assert_eq!(st, Status::OK, "atanPi(-0) [{rm:?}] flags");

        assert_exact(
            Decimal64::INFINITY.atan_pi(rm),
            HALF,
            &format!("atanPi(+inf) [{rm:?}]"),
        );
        assert_exact(
            Decimal64::NEG_INFINITY.atan_pi(rm),
            NEG_HALF,
            &format!("atanPi(-inf) [{rm:?}]"),
        );

        let (r, st) = Decimal64::NAN.atan_pi(rm);
        assert!(r.is_nan() && st.is_ok(), "atanPi(NaN) [{rm:?}]");
        let (r, st) = Decimal64::SIGNALING_NAN.atan_pi(rm);
...
fn unit_rows_are_exact_across_cohorts() {
    ...
    for label in ["1.000000000000001", "0.9999999999999999"] {
        for rm in ALL {
...
fn tiny_arguments_are_generic() {
    ...
    for label in ["1e-20", "1e-100", "7e-380"] {
        let x = parse(label);
        for rm in ALL {
            let (r, st) = x.atan_pi(rm);
            assert!(st.inexact(), "atanPi({label}) [{rm:?}] flags {st:?}");
