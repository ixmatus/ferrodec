//! Exact-result, tie, and special-value gate for `Decimal64::exp2_m1`
...
//! file is the claim's exhaustive witness at 16 digits: every `n` in
//! `1..=53` and every `m` in `1..=16`, in all five rounding
...
//! `n = 56` is the single multiple of four whose value spans 17
//! digits; `10^m − 5^m` always ends in 5, so `m = 17` is a midpoint
...
/// The two tie inputs at this format: `2^56 − 1` is the one 17-digit
/// value with `4 | n`, and `m = PRECISION + 1 = 17` is always one.
const TIE_POS: u32 = 56;
const TIE_NEG: u32 = 17;

/// The exact midpoint `2^56 − 1`, spelled out.
const TIE_POS_MID: &str = "72057594037927935";
/// Its neighbour of larger magnitude (`…794E+1`, the even one).
const TIE_POS_UP: &str = "72057594037927940";
/// Its neighbour of smaller magnitude (`…793E+1`).
const TIE_POS_DOWN: &str = "72057594037927930";

/// The exact midpoint `2^−17 − 1`, spelled out.
const TIE_NEG_MID: &str = "-0.99999237060546875";
/// Its neighbour of larger magnitude (the even one).
const TIE_NEG_UP: &str = "-0.9999923706054688";
/// Its neighbour of smaller magnitude.
const TIE_NEG_DOWN: &str = "-0.9999923706054687";

/// The representable value one step toward zero from `−1`.
const MINUS_ONE_NEIGHBOUR: &str = "-0.9999999999999999";
...
/// Every `exp2m1(n) = 2^n − 1` in reach of 16 digits, all five modes.
#[test]
...
/// Every `exp2m1(−m) = −(10^m − 5^m)·10^−m` in reach of 16 digits, all
/// five modes.
#[test]
...
fn exact_family_matches_the_independent_witness() {
    for n in 1..=N_MAX {
        ...
        let (sum, st) = r.add(Decimal64::ONE, NE);
...
/// The two `Decimal64` rows of the derivation's tie table, literal,
...
/// The negative-side `Decimal64` tie row; see
...
fn minus_one_band_spot_rows() {
    ...
    for label in ["-200", "-1000", "-1e6", "-1e100", "-9.999999e384"] {
        let x = parse(label);
        for (rm, want) in [
            (NE, Decimal64::NEG_ONE),
            (NA, Decimal64::NEG_ONE),
            (TZ, neighbour),
            (TP, neighbour),
            (TN, Decimal64::NEG_ONE),
        ] {
            let (r, st) = x.exp2_m1(rm);
            assert!(
                eq(r, want),
    ...
    for label in ["-160", "-170"] {
        ...
        assert!(eq(r, Decimal64::NEG_ONE), "exp2_m1({label}) = {r}");
...
/// The overflow band: past `x ≈ +1280` the §7.4 disposition applies
/// per direction.
#[test]
fn overflow_band_spot_rows() {
    for label in ["2000", "1e6", "1e100", "9.999999e384"] {
        let x = parse(label);
        for rm in ALL {
            let (r, st) = x.exp2_m1(rm);
            assert!(
...
fn specials_per_section_9_2_1() {
    for rm in ALL {
        let (r, st) = Decimal64::ZERO.exp2_m1(rm);
        assert!(r.is_zero() && !r.is_sign_negative(), "exp2_m1(+0) = {r}");
        assert_eq!(st, Status::OK, "exp2_m1(+0) status {st:?}");
        let (r, st) = Decimal64::NEG_ZERO.exp2_m1(rm);
        assert!(r.is_zero() && r.is_sign_negative(), "exp2_m1(-0) = {r}");
        assert_eq!(st, Status::OK, "exp2_m1(-0) status {st:?}");

        let (r, st) = Decimal64::NEG_INFINITY.exp2_m1(rm);
        assert!(eq(r, Decimal64::NEG_ONE), "exp2_m1(-inf) = {r}");
        assert_eq!(st, Status::OK, "exp2_m1(-inf) status {st:?}");

        let (r, st) = Decimal64::INFINITY.exp2_m1(rm);
        assert!(
            r.is_infinite() && !r.is_sign_negative(),
            "exp2_m1(+inf) = {r}"
        );
        ...
        let (r, st) = Decimal64::NAN.exp2_m1(rm);
        assert!(r.is_nan() && st.is_ok(), "exp2_m1(NaN) = {r} {st:?}");
        let (r, st) = Decimal64::SIGNALING_NAN.exp2_m1(rm);
...
fn near_misses_are_declined() {
    for label in [
        // Non-integers either side of exact integer inputs (16 digits
...
fn tiny_inputs_deliver_without_an_x_anchor_seam() {
    for label in [
        "1e-40", "-1e-40", "1e-100", "-1e-100", "1e-380", "-1e-380", "1e-398", "-1e-398",
    ] {
        let x = parse(label);
        for rm in ALL {
            let (r, st) = x.exp2_m1(rm);
