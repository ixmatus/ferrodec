...
//! `Decimal64::tan_pi` (IEEE 754-2019 §9.2 `tanPi`; ADR-0061 Track D
//! group D4).
//!
...
//! `|tanPi| = |cot(πδ)| ≤ 1/(π|δ|) ≤ 10^16/π ≈ 3.18·10^15` — 369
...
//! below by `10^-16`, so the tightest hug the format can express is
//! `2π·10^-16 ≈ 6.3·10^-16`, which rung 1 resolves with thirty decades
//! to spare. The kernel carries the ADR-0051 arm ADR-0061 specifies,
//! but its gate (`adj(ε) ≤ −20`) sits three decades below that floor
...
/// `tan(54°) = √(5 + 2√5)` at 16 digits.
const TAN_54: &str = "1.376381920471174";
/// `tan(18°)` at 16 digits.
const TAN_18: &str = "0.3249196962329063";
...
fn specials_per_section_9_2_1() {
    for rm in ALL {
        // tanPi(±0) is ±0.
        assert_zero(
            Decimal64::ZERO.tan_pi(rm),
            false,
            &format!("tanPi(+0) [{rm:?}]"),
        );
        assert_zero(
            Decimal64::NEG_ZERO.tan_pi(rm),
            true,
            &format!("tanPi(-0) [{rm:?}]"),
        );
        ...
        let (r, st) = Decimal64::NAN.tan_pi(rm);
        assert!(r.is_nan() && st.is_ok(), "tanPi(NaN) [{rm:?}] = {r} {st:?}");
        let (r, st) = Decimal64::SIGNALING_NAN.tan_pi(rm);
        assert!(
            r.is_nan() && st.invalid(),
            "tanPi(sNaN) [{rm:?}] = {r} {st:?}"
        );
...
fn exact_at_the_quarter_integers() {
    ...
    for (label, positive) in [
        ...
        // Cohort variants.
        ("0.250", true),
        ("0.2500000000000000", true),
        ("25E-2", true),
        ("75E-2", false),
        // The widest quarter integer this format can spell.
        ("99999999999999.25", true),
        ("99999999999999.75", false),
    ] {
...
/// neighbours of a pole. `|δ| = 10^-16` is the tightest offset the
/// format can spell beside `0.5`, so this is the largest pole value the
/// operation can ever produce: `1/(π·10^-16) ≈ 3.18·10^15`, finite,
/// with 369 decades of headroom. The sign alternates across the pole.
#[test]
fn poles_cannot_overflow() {
    // `10^16/π`, from `1/π = 0.3183098861837906715…`
    // rounded to this format's 16 digits. `cot(t) = 1/t − t/3 − …`, and
    // the correction is `≈ 10^-16`, sixteen decades under the last
    // digit, so the cap is `1/t` to every digit the format carries.
    let cap = parse("3.183098861837907E+15");
    // The global ceiling `10^16/π` over every pole neighbourhood, and a
    // floor loose enough to hold for all of them.
    let global_cap = parse("3.184E+15");
    let floor = parse("1E+14");
    // (operand, expected result sign). The offset a pole neighbour can
    // carry depends on the operand's own adjusted exponent: beside
    // `0.5` it is `10^-16`, beside `1.5` only `10^-15`, because the
    ...
    for (label, neg) in cases {
        for rm in ALL {
            ...
            assert!(
                ...
                "tanPi({label}) [{rm:?}] = {r} left the proven 10^16/π cap"
            );
        }
    }
    // The tightest neighbour the format can spell lands on the derived
    // cap `1/(π·10^-16)`: the largest value this operation can produce.
    let (r, _) = parse("0.4999999999999999").tan_pi(NE);
    assert!(
        eq(r, cap),
        "the pole cap drifted: got {r}, want {cap} (= 1/(π·10^-16))"
    );
}
...
/// `2π·10^-16`, which straddles the `5·10^-16` midpoint above 1 and so
...
fn quarter_integer_hug_resolves_on_the_ladder() {
    let one = parse("1");
    let above = parse("1.000000000000001");
    // Just above 0.25: tan is increasing, so the value passes 1.
    let x = parse("0.2500000000000001");
    for rm in [NE, NA, TP] {
        let (r, st) = x.tan_pi(rm);
    ...
    let y = parse("0.2499999999999999");
    for rm in ALL {
    ...
    // the other side.
    let z = parse("0.7500000000000001");
    for rm in [NE, NA] {
        let (r, _) = z.tan_pi(rm);
        assert!(
