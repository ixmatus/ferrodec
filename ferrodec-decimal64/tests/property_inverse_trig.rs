//! Faithful-rounding contract for `Decimal64` atan / asin / acos /
//! atan2 vs astro-float, asserted for every IEEE 754 rounding
//! direction (ADR-0021, IEEE 754-2019 §9.2). See `tests/common/mod.rs`
//! for the contract; this is not a `± ULP` tolerance envelope.
//!
...
#[test]
fn atan_one() {
    check_unary("atan", "1", Decimal64::atan);
}
#[test]
fn atan_two() {
    check_unary("atan", "2", Decimal64::atan);
}
#[test]
fn atan_huge() {
    check_unary("atan", "1e30", Decimal64::atan);
}
#[test]
fn atan_tiny() {
    check_unary("atan", "1e-30", Decimal64::atan);
}
#[test]
fn atan_half() {
    check_unary("atan", "0.5", Decimal64::atan);
}
#[test]
fn atan_pi() {
    check_unary("atan", "3.141592653589793", Decimal64::atan);
}
...
#[test]
fn asin_half() {
    check_unary("asin", "0.5", Decimal64::asin);
}
#[test]
fn asin_neg_half() {
    check_unary("asin", "-0.5", Decimal64::asin);
...
#[test]
fn asin_near_one() {
    check_unary("asin", "0.999", Decimal64::asin);
}
#[test]
fn asin_tiny() {
    check_unary("asin", "1e-15", Decimal64::asin);
}
...
#[test]
fn acos_half() {
    check_unary("acos", "0.5", Decimal64::acos);
}
#[test]
fn acos_quarter() {
    check_unary("acos", "0.25", Decimal64::acos);
}
#[test]
fn acos_neg_half() {
    check_unary("acos", "-0.5", Decimal64::acos);
...
fn check_atan2(y_str: &str, x_str: &str) {
    ...
    // suite uses. The sign bits come from the parsed Decimal64 values
...
proptest! {
    ...
    /// the sweep rather than skipped after the fact — no bracket
    /// weakening, the in-domain bracket stays the full 5-mode
    /// contract.
    #[test]
    fn asin_acos_random_faithful(
        ...
        // `[10^-27, ~1.0)` — always strictly inside the asin/acos
