//! Exact correctly-rounded oracle for `Decimal64::round_to_integral`
...
//! ([`decode_decimal64`], itself sanity-checked as the exact inverse of
...
use ferrodec_test_support::oracle::{decode_decimal64, round_up_decision};
...
fn oracle_integral(x: Decimal64, rm: RoundingMode, exact_variant: bool) -> Pred {
    ...
    let (neg, coeff_big, exp) = decode_decimal64(x.to_bits());
    if exp >= 0 {
    ...
    // A Form-B encoding whose raw coefficient is ≥ 10^16 is
    // non-canonical and, per the BID-64 layout (`src/bid.rs`), denotes
    // ±0 with the encoded sign and exponent — exactly how the
    // production decoder canonicalises it. `decode_decimal64` returns
    ...
    let is_zero = coeff_val == 0 || coeff_val >= 10u128.pow(16);
...
fn check(x: Decimal64, rm: RoundingMode, exact_variant: bool) -> Result<(), TestCaseError> {
    ...
    match want {
        ...
        Pred::Unchanged => {
            prop_assert!(
                ...
                "round_to_integral{variant}({x:e}) rm={rm:?}: expected unchanged {:#018x}, got {:#018x}",
                x.to_bits(),
                got.to_bits()
            );
        }
        Pred::Cohort { neg, coef, .. } => {
            let (gneg, gcoef, gexp) = decode_decimal64(got.to_bits());
            prop_assert!(
...
fn finite() -> impl Strategy<Value = Decimal64> {
    ...
    let structured = (
        any::<bool>(),
        0u64..=9_999_999_999_999_999u64,
        -20i32..=5i32,
    )
        .prop_map(|(s, c, e)| {
            let sign = if s { "-" } else { "" };
            Decimal64::parse_str(&format!("{sign}{c}E{e}"), RoundingMode::NearestEven)
