//! `Decimal64::ln_1p` (IEEE 754-2019 §9.2 `logp1`): special values,
...
/// gate uses).
fn eq(a: Decimal64, b: Decimal64) -> bool {
...
fn zeros_return_themselves_sign_preserved_and_exception_free() {
    for rm in ALL {
        let (r, s) = Decimal64::ZERO.ln_1p(rm);
        assert!(
        ...
        let (r, s) = Decimal64::NEG_ZERO.ln_1p(rm);
        assert!(
            r.is_zero() && r.is_sign_negative(),
            "logp1(-0) = -0 [{rm:?}]"
        );
...
fn minus_one_is_negative_infinity_with_div_by_zero() {
    for rm in ALL {
        let (r, s) = Decimal64::NEG_ONE.ln_1p(rm);
        assert!(
...
fn below_minus_one_is_invalid_nan() {
    ...
    let just_below = Decimal64::NEG_ONE.next_down().0;
    for rm in ALL {
        ...
        let (r, s) = Decimal64::NEG_INFINITY.ln_1p(rm);
...
fn just_above_minus_one_stays_in_domain() {
    ...
    let x = Decimal64::NEG_ONE.next_up().0;
    for rm in ALL {
        let (r, s) = x.ln_1p(rm);
...
fn positive_infinity_passes_through() {
    for rm in ALL {
        let (r, s) = Decimal64::INFINITY.ln_1p(rm);
        assert!(
            r.is_infinite() && !r.is_sign_negative(),
            "logp1(+inf) [{rm:?}]"
        );
...
/// `t` values whose `t − 1` is exactly representable at 16 digits, so
...
fn agrees_with_ln_wherever_one_plus_x_is_representable() {
    for lit in LN_CROSS_CHECK_T {
        let t = parse(lit);
        let (x, st) = t.sub(Decimal64::ONE, NE);
        assert!(
            !st.inexact(),
            "test premise: {lit} - 1 must be exact, got status {st:?}"
        );
        let (back, _) = Decimal64::ONE.add(x, NE);
        assert!(
            eq(back, t),
...
fn anchor_band_both_signs_follows_the_side_theorem() {
    ...
    check_anchor("1e-40");
    check_anchor("2.718281828459045e-40");
    // Snap band, normal results.
    check_anchor("1e-50");
    check_anchor("1e-100");
    check_anchor("1e-380");
    // Snap band, subnormal results (Decimal64 turns subnormal below
    // 1e-383): the UNDERFLOW leg of the assertion above.
    check_anchor("1e-390");
    check_anchor("1e-395");
...
fn in_domain() -> impl Strategy<Value = Decimal64> {
    prop_oneof![
        (1u64..10u64.pow(16), -60i32..=60i32)
            .prop_map(|(coef, exp)| parse(&format!("{coef}e{exp}"))),
        (1u64..10u64.pow(16), 0i32..=60i32).prop_map(|(coef, shift)| {
