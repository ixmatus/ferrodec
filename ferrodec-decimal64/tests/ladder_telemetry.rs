...
//! ULP of a rounding boundary. At P = 16 that threshold is below
//! the resolution the format's own coefficient lattice can express,
//! so NO Decimal64 input can naturally escalate on any operation --
...
fn sibling_corpus_never_escalates() {
    assert!(
        ...
        "planted corpus grew Decimal64 rows; the generator derivation says \
    ...
    assert_eq!(
        ...
        0,
        "a Decimal64 corpus row naturally escalated: the threshold \
         derivation (B x 10^(16 - 50) fractional ULP, below the \
