...
//! for `Decimal64`'s `powi` (IEEE 754-2019 §9.2 `pown`; ADR-0059
...
const EMAX: i32 = 384;
/// Smallest decimal exponent of a representable `Decimal64`.
const ETINY: i32 = -398;
...
fn row_n_zero_is_one_unless_the_base_signals() {
    for x in [
        ...
        for rm in ALL {
            ...
            assert!(equal(r, Decimal64::ONE), "powi({x}, 0) at {rm:?} = {r}");
            assert_eq!(st, Status::OK, "powi({x}, 0) at {rm:?}: flags");
        }
    }
    for rm in ALL {
        let (r, st) = Decimal64::SIGNALING_NAN.powi(0, rm);
...
fn row_zero_base_negative_n_divides_by_zero() {
    for n in [-1, -3, -7, -49, i32::MAX.wrapping_neg()] {
        ...
        for (x, want_neg) in [(Decimal64::ZERO, false), (Decimal64::NEG_ZERO, true)] {
            for rm in ALL {
...
fn row_infinite_base() {
    for n in [1, 3, 49, i32::MAX] {
        for rm in ALL {
            let (r, st) = Decimal64::INFINITY.powi(n, rm);
            assert!(r.is_infinite() && !r.is_sign_negative(), "powi(+inf, {n})");
            assert_eq!(st, Status::OK);
            let (r, st) = Decimal64::NEG_INFINITY.powi(n, rm);
            assert!(r.is_infinite() && r.is_sign_negative(), "powi(-inf, {n})");
            assert_eq!(st, Status::OK);
        }
    }
    for n in [2, 4, 112] {
        for rm in ALL {
            let (r, _) = Decimal64::INFINITY.powi(n, rm);
            assert!(r.is_infinite() && !r.is_sign_negative(), "powi(+inf, {n})");
            let (r, _) = Decimal64::NEG_INFINITY.powi(n, rm);
            assert!(
                r.is_infinite() && !r.is_sign_negative(),
...
/// `2^53` is exactly 16 digits: the widest exact power of two the
/// format holds, delivered clean in every direction. `2^54` needs 17
/// and is the first inexact one, where the dropped digit is a 4 —
/// below the midpoint, so the three "down" modes keep the truncation
/// and only `TowardPositive` steps up.
#[test]
fn two_to_the_53_is_exact_and_54_is_not() {
    let want = parse("9007199254740992");
    for rm in ALL {
        let (r, st) = parse("2").powi(53, rm);
        assert!(equal(r, want), "powi(2, 53) at {rm:?} = {r}");
        assert_eq!(st, Status::OK, "powi(2, 53) at {rm:?}: flags");
    }
    let down = parse("1.801439850948198E+16");
    let up = parse("1.801439850948199E+16");
    for (rm, w) in [(NE, down), (NA, down), (TZ, down), (TN, down), (TP, up)] {
        let (r, st) = parse("2").powi(54, rm);
        assert!(equal(r, w), "powi(2, 54) at {rm:?} = {r}, want {w}");
        assert!(st.inexact(), "powi(2, 54) at {rm:?}: want INEXACT");
        assert!(!st.overflow(), "powi(2, 54) at {rm:?}: no overflow");
    }
}

/// The real tie. `5^24` has exactly 17 digits and ends in 5, so the
...
/// the truncation.
#[test]
fn the_tie_at_five_to_the_24_resolves_by_mode() {
    let down = parse("5.960464477539062E+16");
    let up = parse("5.960464477539063E+16");
    for (rm, w, label) in [
        ...
        let (r, st) = parse("5").powi(24, rm);
        assert!(equal(r, w), "powi(5, 24) at {rm:?} = {r}: {label}");
        assert!(st.inexact(), "powi(5, 24) at {rm:?}: want INEXACT");
    }
    // The negative-exponent mirror: 2^-24 = 5^24 · 10^-24, the same
    // 17-digit coefficient one decade family down.
    for (rm, w) in [(NE, "5.960464477539062E-8"), (NA, "5.960464477539063E-8")] {
        let (r, st) = parse("2").powi(-24, rm);
        assert!(equal(r, parse(w)), "powi(2, -24) at {rm:?} = {r}");
    ...
    // magnitude.
    let (r, _) = parse("-5").powi(23, TN);
    let (m, _) = parse("5").powi(23, TP);
    assert!(equal(r, m.neg()), "powi(-5, 23) at TowardNegative = {r}");
    let (r, _) = parse("-5").powi(23, TP);
    let (m, _) = parse("5").powi(23, TZ);
    assert!(equal(r, m.neg()), "powi(-5, 23) at TowardPositive = {r}");
}
...
fn n_plus_minus_one_is_identity_and_reciprocal() {
    ...
    for literal in ["3", "7", "-3", "1.7", "9.87654321"] {
        ...
        for rm in ALL {
            let (r, st) = x.powi(-1, rm);
            let (d, ds) = Decimal64::ONE.div(x, rm);
            assert!(
                equal(r, d),
...
fn above_range_overflows_per_mode() {
    let mut cases: Vec<(Decimal64, i32, &str)> = Vec::new();
    for n in [
        EMAX + 1,
        EMAX + 2,
        10_000,
    ...
    cases.push((parse("1e200"), 6, "1e200"));
    cases.push((parse("1.7e200"), 6, "1.7e200"));
    cases.push((parse("1.7e200"), 7, "1.7e200"));
    cases.push((parse("1e100"), 2000, "1e100"));
...
fn below_range_underflows_per_mode() {
    ...
    cases.push((parse("1e-200"), 6, "1e-200"));
    cases.push((parse("1.7e-200"), 6, "1.7e-200"));
    cases.push((parse("1.7e-200"), 7, "1.7e-200"));
    cases.push((parse("1e100"), -2000, "1e100"));
    for (x, n, label) in cases {
...
fn exact_results_in_the_subnormal_band_are_clean() {
    ...
    // 5^3 · 10^-399 needs a quantum one below `etiny`: inexact, and
    // tiny, so both flags are owed.
    for (base, n) in [("5e-133", 3), ("3e-200", 2)] {
...
fn the_powering_arms_reciprocal_survives_out_of_range_accumulators() {
    for (base, n) in [("1.7e200", -6), ("1.7e150", -5), ("9.9e384", -6)] {
        for rm in [NE, NA, TZ, TN] {
    ...
    for (base, n) in [("1.7e-200", -6), ("1.7e-150", -5), ("9e-398", -6)] {
        for rm in [NE, NA, TP] {
            let (r, st) = parse(base).powi(n, rm);
            assert!(
                r.is_infinite() && !r.is_sign_negative(),
    ...
    for rm in [NE, NA, TN] {
        let (r, st) = parse("-1.7e-200").powi(-5, rm);
        assert!(
            r.is_infinite() && r.is_sign_negative(),
            "powi(-1.7e-200, -5) at {rm:?} = {r}, want -inf"
        );
        assert!(st.overflow() && st.inexact(), "flags at {rm:?}: {st:?}");
    }
    for rm in [TZ, TP] {
        let (r, _) = parse("-1.7e-200").powi(-5, rm);
        assert!(
            equal(r, Decimal64::MAX.neg()),
            "powi(-1.7e-200, -5) at {rm:?} = {r}, want -MAX"
        );
    }
}
...
fn the_powering_arm_composes() {
    for base in ["1.5", "-1.5", "2", "0.2", "3", "-0.5", "1.25"] {
        ...
        let (recip, _) = Decimal64::ONE.div(pos, NE);
        assert!(
            equal(neg, recip),
...
fn unit_bases_at_every_exponent() {
    for n in [1, -1, 2, -2, 7, -7, 99_999, -99_999, i32::MAX, i32::MIN] {
        for rm in ALL {
            let (r, st) = Decimal64::ONE.powi(n, rm);
            assert!(equal(r, Decimal64::ONE), "powi(1, {n}) at {rm:?} = {r}");
            assert_eq!(st, Status::OK, "powi(1, {n}): flags");
            let (r, st) = Decimal64::NEG_ONE.powi(n, rm);
            let want = if n % 2 == 0 {
                Decimal64::ONE
            } else {
                Decimal64::NEG_ONE
