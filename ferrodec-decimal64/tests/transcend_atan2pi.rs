...
//! `Decimal64::atan2_pi` (IEEE 754-2019 §9.2 `atan2Pi`; ADR-0061
...
//! two anchor gates are `adj(y) - adj(x) ≥ 18` and `≤ -19`.
//!
...
fn specials_per_section_9_2_1() {
    ...
    let ninf = Decimal64::NEG_INFINITY;
    let zero = Decimal64::ZERO;
    let nzero = Decimal64::NEG_ZERO;
    for rm in ALL {
        assert_exact(
            inf.atan2_pi(inf, rm),
            "0.25",
        ...
        let (r, st) = Decimal64::NAN.atan2_pi(parse("1"), rm);
        assert!(r.is_nan() && st.is_ok(), "(NaN,1) {rm:?}");
        let (r, st) = parse("1").atan2_pi(Decimal64::SIGNALING_NAN, rm);
...
fn axis_rows_are_exact_where_atan2_was_inexact() {
    ...
    for (y, x) in [
        ...
        (inf, parse("1")),
        (parse("1"), ninf),
        (Decimal64::ZERO, parse("-1")),
        (parse("1"), Decimal64::ZERO),
    ] {
        for rm in ALL {
...
fn diagonals_are_exact() {
    ...
    for (y, x) in [("1.000000000000001", "1"), ("1", "-1.000000000000001")] {
        for rm in ALL {
...
fn half_turn_band_takes_its_side_from_the_abscissa() {
    for gap in [HALF_GATE, HALF_GATE + 10, 200] {
...
fn full_turn_band_hugs_from_inside() {
    for gap in [FULL_GATE, FULL_GATE - 10, -200] {
        ...
        for (rm, want) in [
            (NE, "-1"),
            (TZ, "-0.9999999999999999"),
            (TP, "-0.9999999999999999"),
            (TN, "-1"),
        ] {
