...
/// [`Decimal64::round_to_integral_special_only_for_kani`] can route
...
fn round_to_integral_special_cases(x: Decimal64) -> Option<(Decimal64, Status)> {
    ...
    match classify_bits(x.to_bits()) {
        Class::Finite {
            ...
            if drop >= digits {
                ...
                let coef_out = u64::from(round_up);
            ...
            // Carry: if `new_coef` reached 10^16, normalise by shifting
            // up one decade. The coefficient is now 10^15 with quantum
...
mod tests {
    ...
    fn d_eq(a: Decimal64, b: Decimal64) -> bool {
    ...
    #[test]
    fn round_specials() {
        let (n, _) = Decimal64::NAN.round_to_integral(RoundingMode::NearestEven);
        assert!(n.is_nan());
        let (s, st) = Decimal64::SIGNALING_NAN.round_to_integral(RoundingMode::NearestEven);
        assert!(s.is_nan());
        assert!(st.invalid());
        let (i, _) = Decimal64::INFINITY.round_to_integral(RoundingMode::NearestEven);
    ...
    fn floor_preserves_sign_of_zero() {
        let neg_zero = Decimal64::NEG_ZERO;
        assert!(neg_zero.floor().is_sign_negative());
        assert!(neg_zero.ceil().is_sign_negative());
        assert!(!Decimal64::ZERO.floor().is_sign_negative());
    }
}
