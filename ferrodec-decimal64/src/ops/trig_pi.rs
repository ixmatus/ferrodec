...
//! `ferrodec-transcend` (ADR-0061 Track D D4). The public `Decimal64`
...
impl Decimal64 {
    ...
    pub fn asin_pi(self, rm: RoundingMode) -> (Self, Status) {
        ferrodec_transcend::inverse_trig_pi::asin_pi_kernel::<Decimal64>(self, rm)
    }
    ...
    pub fn acos_pi(self, rm: RoundingMode) -> (Self, Status) {
        ferrodec_transcend::inverse_trig_pi::acos_pi_kernel::<Decimal64>(self, rm)
    }
    ...
    pub fn atan_pi(self, rm: RoundingMode) -> (Self, Status) {
        ferrodec_transcend::inverse_trig_pi::atan_pi_kernel::<Decimal64>(self, rm)
    }
    ...
    pub fn atan2_pi(self, x: Self, rm: RoundingMode) -> (Self, Status) {
        ferrodec_transcend::inverse_trig_pi::atan2_pi_kernel::<Decimal64>(self, x, rm)
    }
}
...
mod tests {
    ...
    fn quarter_turns_are_exact() {
        for (got, want) in [
            ...
            (Decimal64::INFINITY.atan_pi(NE), "0.5"),
        ] {
            let (r, st) = got;
    ...
    fn one_sixth_is_inexact() {
        ...
        assert!(equal(r, parse("0.1666666666666667")), "got {r}");
        assert!(st.inexact());
    }
