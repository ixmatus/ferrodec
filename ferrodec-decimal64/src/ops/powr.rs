//! IEEE 754-2019 §9.2 `powr` for [`Decimal64`] (ADR-0059 Track D D3).
...
impl Decimal64 {
    ...
    /// [`Decimal64::pow`] on four families of input where taking the
    ...
    pub fn powr(self, exponent: Self, rm: RoundingMode) -> (Self, Status) {
        ferrodec_transcend::powr::powr_kernel::<Decimal64>(self, exponent, rm)
    }
    ...
    /// [`Decimal64::pow_special_only_for_kani`]. `None` for the
    /// general path (`x` finite `> 0`, `y` finite non-zero).
    #[cfg(kani)]
    #[doc(hidden)]
    #[must_use]
    pub fn powr_special_only_for_kani(self, exponent: Self) -> Option<(Self, Status)> {
        ferrodec_transcend::powr::powr_special_cases::<Decimal64>(self, exponent)
    }
}
