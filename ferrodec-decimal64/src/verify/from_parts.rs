//! Kani harnesses for [`Decimal64::from_parts`], the inverse of
//! [`Decimal64::decode`].
//!
...
//! Because it is a pure encode (no arithmetic), the full `(bool, u64, i16)`
...
fn from_parts_in_range_roundtrips() {
    ...
    let coefficient: u64 = kani::any();
    let exponent: i16 = kani::any();
    let parts = Decimal64Parts {
        negative,
        coefficient,
        exponent,
    };
    ...
    match Decimal64::from_parts(parts) {
        Some(d) => {
            assert!(in_range);
...
fn decode_then_from_parts_bit_identity() {
    ...
    let r = Decimal64::from_parts(p).expect("decoded canonical parts re-encode");
...
fn from_parts_out_of_range_is_none() {
    ...
    assert!(Decimal64::from_parts(parts).is_none());
}
