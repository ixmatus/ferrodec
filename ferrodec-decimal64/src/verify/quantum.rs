...
//! `Decimal64::{quantize, scaleb, logb, next_up, next_down}`.
//!
...
fn quantize_nan_and_infinity() {
    ...
    let (r, s) = Decimal64::NEG_INFINITY
        .quantize_special_only_for_kani(Decimal64::INFINITY)
...
fn logb_special_boundary() {
    ...
    let (r, _) = Decimal64::NEG_INFINITY
        .logb_special_only_for_kani()
...
fn next_up_special_boundary() {
    ...
    assert!(r.to_bits() == Decimal64::MIN_POSITIVE.to_bits() && !s.invalid());

    let (r, _) = Decimal64::NEG_INFINITY
        .next_up_special_only_for_kani()
        .expect("−∞ resolved by next_up_special_cases");
    assert!(r.to_bits() == Decimal64::MIN.to_bits());
...
fn next_down_special_boundary() {
    ...
    assert!(
        r.to_bits() == Decimal64::MIN_POSITIVE.neg().to_bits()
            && r.is_sign_negative()
            && !s.invalid()
    );
