            ...
            b'0'..=b'9' => {
                ...
                if digits_total < MAX_PARSED_DIGITS {
                    ...
                    if !decimal_seen {
                        ...
                        // miscomputes the exponent. A literal-length
                        // cap, unlike the explicit-exponent field,
                        // which saturates instead of rejecting
                        // (ADR-0057); ADR-0029 item 2 / fd-7f1
...
mod tests {
    ...
    fn parse_rounding_at_precision_boundary() {
        ...
        let expected = Decimal64::from_bits(pack_finite(
            false,
            BiasedExp::try_from_biased(BIAS + 1).unwrap(),
            Coefficient::try_new(1_234_567_890_123_457).unwrap(),
        ));
        assert_eq!(d.to_bits(), expected.to_bits());
