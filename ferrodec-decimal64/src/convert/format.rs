//! [`Decimal64`] → string formatting via `core::fmt::Write`. Alloc-free.
//!
...
impl fmt::LowerExp for Decimal64 {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        format_to(*self, f, Notation::ScientificForced('e'))
    }
}

impl fmt::UpperExp for Decimal64 {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        format_to(*self, f, Notation::ScientificForced('E'))
    }
}
...
/// `digits` total) into an 18-byte stack buffer, returning a fixed
/// array with the digits as ASCII.
fn digit_string(coef: u64, digits: u32) -> [u8; 18] {
    // 16 digits max for canonical Decimal64; allocate 18 to give room
    // for a transient overflow during rendering of pre-rounded
    // coefficients.
    let mut buf = [0u8; 18];
    let mut n = coef;
...
mod tests {
    ...
    fn parse(s: &str) -> Decimal64 {
        Decimal64::parse_str(s, RoundingMode::default()).unwrap().0
    }

    #[test]
    fn display_zero() {
        assert_eq!(Decimal64::ZERO.to_string(), "0");
        assert_eq!(Decimal64::NEG_ZERO.to_string(), "-0");
    }

    #[test]
    fn display_one_and_neg_one() {
        assert_eq!(Decimal64::ONE.to_string(), "1");
        assert_eq!(Decimal64::NEG_ONE.to_string(), "-1");
    }

    #[test]
    fn display_specials() {
        assert_eq!(Decimal64::INFINITY.to_string(), "Infinity");
        assert_eq!(Decimal64::NEG_INFINITY.to_string(), "-Infinity");
        assert_eq!(Decimal64::NAN.to_string(), "NaN");
        assert_eq!(Decimal64::SIGNALING_NAN.to_string(), "sNaN");
    }
    ...
    fn display_zero_with_positive_exp_is_scientific() {
        ...
        let d = Decimal64::try_new(0, 5).unwrap();
