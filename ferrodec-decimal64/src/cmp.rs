//! IEEE 754-2019 comparison and ordering for [`Decimal64`].
//!
//! * [`Decimal64::partial_cmp`] — §5.6.1 numeric comparison. Returns
//!   `(Option<Ordering>, Status)` so signaling-NaN inputs can raise
//!   `INVALID` while the comparison still reports incomparability via
//!   `None`.
//! * [`Decimal64::total_cmp`] — §5.10 totalOrder predicate. Returns
//!   `Ordering` directly (always defined, NaN payload-aware).
//! * [`Decimal64::compare_total_magnitude`] — §5.10
//!   totalOrderMag: the totalOrder predicate applied to `|a|` vs
//!   `|b|`.
//! * [`Decimal64::min`] / [`Decimal64::max`] — §5.3.1 minimum and
//!   maximum operations. NaN propagates per the §6.2.3 rule.
//! * [`Decimal64::min_magnitude`] / [`Decimal64::max_magnitude`] —
...
impl Decimal64 {
    ...
    /// `max(x, y)` — symmetric to [`Decimal64::min`]; matches IEEE
    ...
    /// consistent with [`Decimal64::min`] = `minimumNumber`.
    ///
    /// The operand of smaller magnitude wins, `|x|` and `|y|`
    /// compared *numerically* (cohort is irrelevant). On an
    /// equal-magnitude tie the result defers to [`Decimal64::min`],
    ...
    pub fn min_magnitude(self, other: Self) -> (Self, Status) {
        ...
        // `partial_cmp` (the §5.11 numeric comparison) yields `Some`.
    ...
    /// [`Decimal64::min_magnitude`], deferring to [`Decimal64::max`]
...
fn finite_magnitude_cmp(coef_a: u64, exp_a: i32, coef_b: u64, exp_b: i32) -> Ordering {
    ...
    let (na, nb) = if da < db {
        ...
        (coef_a * factor, coef_b)
    } else {
        let factor = 10u64.pow(da - db);
        (coef_a, coef_b * factor)
    };
    na.cmp(&nb)
}
...
fn total_cmp_inner(a: Decimal64, b: Decimal64) -> Ordering {
...
mod tests {
    ...
    fn partial_cmp_zero_cohorts_numerically_equal() {
        // +0 == -0
        let (cmp, _) = Decimal64::ZERO.partial_cmp(Decimal64::NEG_ZERO);
        assert_eq!(cmp, Some(Ordering::Equal));

        // 0E+0 == 0E-1 (numerically)
        let z1 = Decimal64::try_new(0, 0).unwrap();
        let z2 = Decimal64::try_new(0, -1).unwrap();
    ...
    fn partial_cmp_infinity() {
        let (cmp, _) = Decimal64::INFINITY.partial_cmp(from_int(1, 0));
        assert_eq!(cmp, Some(Ordering::Greater));

        let (cmp, _) = Decimal64::NEG_INFINITY.partial_cmp(from_int(1, 0));
        assert_eq!(cmp, Some(Ordering::Less));

        let (cmp, _) = Decimal64::INFINITY.partial_cmp(Decimal64::INFINITY);
        assert_eq!(cmp, Some(Ordering::Equal));
    }

    #[test]
    fn partial_cmp_nan() {
        let (cmp, s) = Decimal64::NAN.partial_cmp(from_int(1, 0));
        assert_eq!(cmp, None);
        assert!(s.is_ok());

        let (cmp, s) = Decimal64::SIGNALING_NAN.partial_cmp(from_int(1, 0));
        assert_eq!(cmp, None);
        assert!(s.invalid());
    ...
    fn total_cmp_negative_quiet_nan_lowest() {
        ...
        assert_eq!(neg_nan.total_cmp(Decimal64::NEG_INFINITY), Ordering::Less);
        assert_eq!(neg_nan.total_cmp(Decimal64::INFINITY), Ordering::Less);
    }
    ...
    fn total_cmp_zeros_distinguished() {
        // -0 < +0 in totalOrder.
        assert_eq!(
            Decimal64::NEG_ZERO.total_cmp(Decimal64::ZERO),
            Ordering::Less
        );
        assert_eq!(
            Decimal64::ZERO.total_cmp(Decimal64::NEG_ZERO),
            Ordering::Greater
        );
    }
    ...
    fn min_max_zero_signs() {
        // min(+0, -0) = -0
        let (r, _) = Decimal64::ZERO.min(Decimal64::NEG_ZERO);
        assert!(r.is_zero() && r.is_sign_negative());

        // max(+0, -0) = +0
        let (r, _) = Decimal64::ZERO.max(Decimal64::NEG_ZERO);
        assert!(r.is_zero() && !r.is_sign_negative());
    }
    ...
    fn min_max_nan() {
        ...
        // qNaN → NaN.
        let (r, s) = Decimal64::NAN.min(from_int(1, 0));
        assert_eq!(r.to_bits(), from_int(1, 0).to_bits());
        assert!(s.is_ok());

        let (r, s) = from_int(7, 0).max(Decimal64::NAN);
        assert_eq!(r.to_bits(), from_int(7, 0).to_bits());
        assert!(s.is_ok());

        let (r, s) = Decimal64::NAN.min(Decimal64::NAN);
        assert!(r.is_quiet_nan());
        assert!(s.is_ok());

        // sNaN still poisons: result is NaN + INVALID regardless of
        // the other operand.
        let (r, s) = Decimal64::SIGNALING_NAN.min(from_int(1, 0));
        assert!(r.is_quiet_nan());
        assert!(s.invalid());
    ...
    fn min_max_magnitude() {
        ...
        // Cohort tie (1.0 vs 1.00): numerically equal magnitude, so
        // the result matches `min` (no panic, correct cohort).
        let a = Decimal64::try_new(10, -1).unwrap();
        let b = Decimal64::try_new(100, -2).unwrap();
        let (m, st) = a.min_magnitude(b);
        assert!(st.is_ok());
        assert_eq!(m.to_bits(), a.min(b).0.to_bits());

        // qNaN missing-value; sNaN poisons.
        let (r, st) = Decimal64::ONE.min_magnitude(Decimal64::NAN);
        assert_eq!(r.to_bits(), Decimal64::ONE.to_bits());
        assert!(st.is_ok());
        let (r, st) = Decimal64::ONE.max_magnitude(Decimal64::SIGNALING_NAN);
        assert!(r.is_nan());
        assert!(st.invalid());
    }
