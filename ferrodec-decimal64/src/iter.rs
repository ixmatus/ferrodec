...
mod tests {
    ...
    fn sum_three_integers() {
        let xs = [
            Decimal64::ONE,
            Decimal64::try_new(2, 0).unwrap(),
            Decimal64::try_new(3, 0).unwrap(),
        ];
        let s: Decimal64 = xs.iter().sum();
        let want = Decimal64::try_new(6, 0).unwrap();
        assert_eq!(s.to_bits(), want.to_bits());
    }

    #[test]
    fn sum_empty_is_zero() {
        let s: Decimal64 = core::iter::empty::<Decimal64>().sum();
        assert!(s.is_zero());
    }
    ...
    fn product_three_integers() {
        let xs = [
            Decimal64::try_new(2, 0).unwrap(),
            Decimal64::try_new(3, 0).unwrap(),
            Decimal64::try_new(4, 0).unwrap(),
        ];
        let p: Decimal64 = xs.into_iter().product();
        let want = Decimal64::try_new(24, 0).unwrap();
        assert_eq!(p.to_bits(), want.to_bits());
    }

    #[test]
    fn product_empty_is_one() {
        let p: Decimal64 = core::iter::empty::<Decimal64>().product();
        assert_eq!(p.to_bits(), Decimal64::ONE.to_bits());
    }
    ...
    fn sum_propagates_nan() {
        ...
        let s: Decimal64 = xs.into_iter().sum();
        assert!(s.is_nan());
    }
}
