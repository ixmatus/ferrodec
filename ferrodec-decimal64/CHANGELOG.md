...
- `Decimal64::exp_m1`, `exp2_m1`, `exp10`, and `exp10_m1` (IEEE
  ...
  `exp2_m1(56)` and `exp2_m1(−17)`; `exp10` exact on
  `[−398, 384]`'s integers; `exp10_m1` exact on the ±16-digit
  nines.
...
- `Decimal64::pow` exact rational powers and ties (ADR-0059 M7), via
  ...
  `TowardNegative` returned `1.999999999999999` with a spurious `INEXACT`
  and now returns the exact `2`, status `OK`, at every rounding direction.
  The 17-digit ties (`pow(2, -23)` and kin) now round away at
  `NearestAway` (`…507813`) where the kernel's error picked the lower
  neighbour. Exact results carry the input-derived cohort.
...
breaking boundary (ADR-0057, ADR-0058). `ddBase` rises 710 -> 922
passes (+38 extreme-exponent, +174 toEng).
...
- `Decimal64::from_parts`, the `const` inverse of `decode`. Reconstructs a
...
- `Decimal64::from_str_const` and the `dec!` macro, for embedding exact
  published constants in `const` initializers
  (`Decimal64::from_str_const("2.99792458e8")` or `dec!("...")`). A `const`
...
- `Decimal64::decode` and the `Decimal64Parts` struct. A quantum preserving
